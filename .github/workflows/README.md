...
| `nightly.yml` | Scheduled (05:00 UTC) full suite — `pytest tests/ parity/` incl. `slow`, `cargo test --features strict-checks` — plus the enforcing criterion / pytest-benchmark regression gate (`tools/check_bench_regression.py`, >10% mean slowdown fails); Testing Standards §2/§4, ETCA-001 T-F7. DV-036: reference SHA 4590d611f34eae5dfcdadb99b562aacf998d6e94 and candidate checked out as equal-length siblings and measured on the same runner in counterbalanced reference→candidate→candidate→reference order with matched dependencies (campaign amendment 4; contention group advisory on hosted runners); raw paired evidence uploaded even on failure; incomplete measurements fail closed. |
