...
.. admonition:: DRAFT — pre-campaign validation evidence only
   :class: important

   **Status:** draft, issued at WP-037 S1 (session ``0145``) against PRIN
   |release|. Definition of Done #3 requires the Parity Report to be *published*
   before ``1.0.0``; this draft is the pre-release form of that document and is
   **not** the published report.

   **What it contains:** every numerical deviation from PRINet 3.0 discovered by
   *validation* work — golden-corpus differential parity, per-primitive parity
   tests written in tandem with each rebuild, kernel-equivalence checks, and the
   Deferred Validation register. Each claim names the artefact that evidences it.

   **What it does not contain:** PRIN's own confirmatory measurements. The full
   benchmark re-run, the capacity sweep, the throughput targets, and every other
   production-scale result are Phase 7 campaign activities that must be
   pre-registered — expected results and failure conditions written down
   **before** execution — under the Experimentation Standards. No pilot,
...
Every quantitative statement carries one of three labels. The distinction is
load-bearing: a validation number says *the rebuild agrees with the reference
within a registered tolerance*, which is a correctness claim; a confirmatory
number says *the system exhibits this measured behavior*, which is a scientific
claim requiring pre-registration.
...
.. list-table::
   ...
   * - **VALIDATION**
     - Differential agreement with ``prinet==3.0.0`` at a tolerance registered
       in Testing Standards §3, or an invariant asserted by a test that runs in
       CI. Reproducible today from the committed corpus and test suite.
     - Present throughout; this is the entire evidentiary basis of the draft.
   * - **CONFIRMATORY**
     - A production-scale measurement of PRIN's own behavior — throughput,
       capacity, accuracy, scaling — produced by a pre-registered Phase 7
       campaign experiment.
     - **None.** Section `Benchmark re-run comparison`_ records the absence
       explicitly rather than substituting a pilot.
   * - **REFERENCE-HISTORICAL**
     - A number measured by the PRINet 3.0 implementation and stored as an
       immutable artefact. Reproduced byte-comparably by ``tools/reproduce.py``,
       but it describes the reference, not PRIN.
     - Present in :doc:`capacity_analysis` and :doc:`paper`, both labelled there.

Tolerance register
------------------

The tolerances below are the registered contract (Testing Standards §3). Loosening
any of them requires a PR note, reviewer sign-off, and an entry in the
numerical-deviation register further down this page. They are stated once, in
code, at ``prin.parity.schema.METRIC_RTOL`` / ``METRIC_ATOL`` and in the corpus
manifest.
...
.. list-table::
   ...
   * - Quantity
     - Tolerance
     - Authority
   * - Trajectories vs the float64 reference
     - ``rtol=1e-6``, ``atol=1e-8``
     - Project Plan §5; ``parity/corpus/manifest.json``
       (``trajectory_rtol`` / ``trajectory_atol``)
   * - Metrics / tensor decompositions, cross-platform corpus regeneration
     - ``rtol=2e-6``, ``atol=1e-12``
     - Project Plan amendments #16, #17
   * - Metrics, single-runtime verification
     - ``rtol=1e-10`` where achievable
     - Testing Standards §3
   * - GPU kernel vs CPU reference (f32)
     - Documented per kernel; default ``rtol=1e-5``, ``atol=1e-6``
     - Testing Standards §3; :doc:`kernel_architecture`
   * - Derivative-level parity on the ``torch.complex64`` affected paths
     - ``1e-6``
     - Project Plan amendment #14 / DV-007; the WP-007 register entry below
   * - Chaotic regimes beyond the shadowing horizon
     - Statistical comparison of order-parameter time series — **never**
       pointwise
     - Testing Standards §3

Golden-corpus differential results (VALIDATION)
-----------------------------------------------

The corpus is the primary parity artefact: 504 cases generated by
``prinet==3.0.0`` itself, each a stored float64 trajectory with a SHA-256 digest
in ``parity/corpus/manifest.json`` and its arrays under
``parity/corpus/cases/``.

.. list-table:: Corpus provenance — ``parity/corpus/manifest.json``
   :header-rows: 1
   :widths: 34 66
   ...
   * - ``created_at``
     - ``2026-08-06T13:58:03.406430+00:00``
   * - ``n_cases``
     - 504
   * - ``trajectory_rtol`` / ``trajectory_atol``
     - ``1e-06`` / ``1e-08``

Coverage of the corpus, by the manifest's own fields:
...
.. list-table::
   ...
   * - Dimension
     - Cases
   * - Model
     - ``kuramoto`` 216, ``hopf`` 216, ``stuart_landau`` 72
   * - Coupling
     - ``full`` 216, ``mean_field`` 144, ``sparse_knn`` 144
   * - Integrator
     - ``euler`` 252, ``rk4`` 252
   * - Oscillator count
     - 8 / 12 / 16 / 24 — 126 cases each
   * - Steps
     - 20 (all cases)
...
correctness corpus, not a scaling study. Large-N behavior, long-horizon drift,
and every performance property are **CONFIRMATORY** questions and belong to the
Phase 7 campaign.

Benchmark re-run comparison
---------------------------

**CONFIRMATORY — not yet run.** Project Plan §6 makes "full re-run of the
benchmark suite" a Phase 7 campaign track, and Definition of Done #4 (GPU and
CPU performance targets met) and #12 (campaign complete, all experiments
pre-registered and reported) cannot be discharged before it happens.

What exists today, and what each piece is:
...
.. list-table::
   ...
   * - Artefact
     - Label
     - What it establishes
   * - ``benchmarks/results/`` (1 JSON + README)
     - —
     - The canonical tracked output directory. It is deliberately near-empty:
       WP-033 built and validated the ``benchrunner`` machinery at
       characterization scale, whose brief named "drawing conclusions from final
       measurements" an explicit non-goal.
   * - ``paper/artefact_manifest.json`` (172 records)
     - REFERENCE-HISTORICAL
     - SHA-256 manifest for the *stored PRINet 3.0* benchmark artefacts. These
       are the reference implementation's measurements; ``tools/reproduce.py``
       renders them byte-comparably. They are not PRIN results.
   * - ``tools/reproduce.py``, ``repro.yml``
     - VALIDATION
     - The reproduction pipeline and its CI gate: byte-comparable regeneration
       from manifest-verified inputs. This validates the *pipeline*, not any
       scientific claim.
   * - ``EVIDENCE/0125-wp032-s1-daemon-latency.json``,
       ``EVIDENCE/0144U-wp036f-s1-controller-provider-report.json``
     - VALIDATION
     - Point measurements taken to discharge a specific engineering question
       (controller provider selection, DirectML execution), each with its
       environment block. Cited in the register entries below; not a benchmark
       campaign.
   * - ``notebooks/*.ipynb`` printed throughputs
     - —
     - API demonstrations at reduced scale. Explicitly not measurements.

The comparison table this section will eventually carry — PRIN vs PRINet 3.0
across the campaign's C1 (parity), C2 (performance), C3 (scientific
replication), and C4 (new capability) tracks — is produced by the Phase 7
campaign synthesis (session ``0194``), not here.

Evidence index
--------------

Where each class of claim in this report is backed:
...
.. list-table::
   ...
   * - Subconscious-controller parity
     - ``parity/test_parity_subconscious.py`` (19 tests),
       ``EVIDENCE/0144U-wp036f-s1-controller-provider-report.json``,
       ``tools/wp036f_reexport_controller.py --check``
   * - Per-primitive Rust parity
     - ``crates/prin-dynamics/tests/parity_*.rs``,
       ``crates/prin-metrics/tests/``, ``crates/prin-tensor/tests/``
   * - Kernel equivalence (GPU vs CPU reference)
     - ``cargo test -p prin-kernels --features wgpu,cpu``; per-kernel tolerances
       in :doc:`kernel_architecture`
   * - Deferred / re-gated items
     - ``DOCS/reports/DEFERRED_VALIDATION_REGISTER.md``, gated by
       ``tools/check_dv_register_gates.py``
   * - Numerical deviations and their dispositions
     - The register below; Project Plan §8.3 amendment log
   * - Device-resident GPU timing re-probe
     - ``EVIDENCE/0144S-wp036e-s3-dv003-timing-reprobe.md``,
       ``tools/dv003_timing_probe.py``
   * - Cycle-level metric trends
     - ``DOCS/reports/NNN-project-state.md`` (one per closed cycle)
