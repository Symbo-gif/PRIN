...
| **T-F4** | **FIXED.** The backlog (WP-036C S1–S4 + EMA-006 + EDA-001 + ETCA-001 + this remediation) was pushed to `origin/main` and CI driven to green. The first push (`a37e846`) surfaced pre-existing failures the never-run CI had masked for the whole phase — a disk-reclaim step that deleted the `setup-python` toolcache (`ccfc442`); a case-sensitivity bug (`docs/` vs `DOCS/` — Linux only); an order-of-tests RNG flake in a slot-attention gradcheck; and two host-timing perf flakes — all fixed or dispositioned in follow-up commits (see the row below). The per-change coverage-measurement gap is recorded as **DV-033**. | `gh run list --branch main` on the final green HEAD: `rust`/`python`/`parity`/`repro`/`snyk` all `success` |
...
| CI-surfaced pre-existing failures (never-run-CI masked, T-F4 follow-ups) | **FIXED / DISPOSITIONED.** (a) `ccfc442`: the T-F5 disk-reclaim step removed `$AGENT_TOOLSDIRECTORY` (= `/opt/hostedtoolcache`), deleting the `actions/setup-python` interpreter the venv points at → `pip: exit 127` (broke `parity`). Reclaim list trimmed to safe targets on all four workflows. (b) `test_acceptance_y2q4.py::TestDocumentation` ×4: path adapted `parents[1] / "docs"` → `"DOCS"` (PRIN's repo layout; PRINet 3.0 used lowercase — masked on the maintainer's case-insensitive Windows FS). Assertions unchanged; the referenced guides exist. (c) `test_train_bridge_slot_attention.py::test_process_frame_gradcheck` (+ siblings): an autouse `torch.manual_seed(0)` fixture — the gradcheck ran on an *unseeded* input through a `match_threshold` greedy-matching branch, so a boundary-landing draw (order-of-tests-dependent) failed the numerical Jacobian on all 6 legs. Testing Standards §1.5. (d) `test_acceptance_y4q1_5.py::test_deterministic_seed`: wall-clock-throughput frame-count flake — folded into **DV-032**, `conftest.py` governed skip. (e) Windows `python.yml` build kept its original torch-install order (reordering shifted the resolved torch version). | `pytest tests/test_acceptance_y2q4.py tests/test_train_bridge_slot_attention.py tests/test_acceptance_y4q1_5.py` → 106 passed / 1 skipped; final CI push all green |

**Post-remediation verdict: PASS** (all three D2 findings `FIXED`; all five D3
and both D4 findings `FIXED` or tracked with a dated DV disposition; the
backlog pushed and `origin/main` CI driven green).

**`origin/main` CI after the remediation** (final HEAD): `rust`, `python`,
`parity`, `repro`, `snyk` all **`success`** — the first green CI run against
any of the WP-036C S1–S4 / EMA-006 / EDA-001 / ETCA-001 range (T-F4 closed).
`gpu` skipped (no `[gpu]` tag); `nightly` runs on its schedule, not on push.
Getting there took three push cycles: `a37e846` (the ten findings), `ccfc442`
(toolcache-deletion hotfix), and the pre-existing-failure fixes above — the
latter two are exactly the class of defect the never-run CI (T-F4) had
concealed for the whole phase.
...
| Governed skips (`conftest.py`) | DV-031(A/B), amdt #41, M5 RNG, + DV-032 quarantine (T-F8, and a timing flake added in remediation) | one auditable hook |
...
| CI workflows | 7 at audit time → **8** after remediation (`nightly.yml` added, T-F7) | all present |
| `schedule:`d workflows | 0 → **1** (`nightly.yml`, T-F7 remediation) | nightly full-suite obligation now met |
| Enforcing bench-regression gates | 0 → **1** (`nightly.yml` `bench-regression`, `tools/check_bench_regression.py`, T-F7 remediation) | >10 % mean slowdown fails |
| `origin/main` CI (latest push `0313af5`) | 2 green / 3 red at audit time | `parity`✅ `snyk`✅; `rust`❌ `python`❌ `repro`❌ — **after the remediation push series: all 5 `success`** (T-F4 closed) |
| Unpushed commits (local `main` ahead) | 28 at audit time → **0** after the remediation pushes | WP-036C S1–S4 + EMA-006 + EDA-001 + ETCA-001 + remediation (T-F4) |
