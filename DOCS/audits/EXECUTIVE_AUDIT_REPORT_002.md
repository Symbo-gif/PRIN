...
- **E-F5 (environment) — RESOLVED 2026-08-08:** Snyk MCP server restored for local Qwen Code sessions (`mcpServers.snyk` in the user Qwen settings, `npx -y snyk@1.1304.2 mcp -t stdio`, mirroring the known-good Windsurf config and CI-pinned CLI version; smoke test registered 12 tools; auth via the existing CLI session, no token in config). Interim gate until verified in a live session: pip-audit + cargo audit + `python.yml` Snyk SCA job.
- **Dependabot alerts on `main` (security posture) — PR #6 OPENED 2026-08-08:** the six open pyo3 alerts exist because the default branch predates the WP-001-F1 pyo3 0.29.0 upgrade; the audited branch is not affected. Executed call (maintainer-delegated): `origin/main` merged into the branch (content no-op; satisfies strict status checks without rewriting session commits) and PR #6 (`feat/wp006-oscillator-state` → `main`) opened with full audit-trail references; alerts auto-close on merge. Branch protection (16 required checks + 1 approving review + linear history) leaves the final approval/merge to the maintainer; rebase-and-merge recommended to preserve the per-session commit audit trail.
...
- **Post-audit CI note (PR #6):** the executive audit verified E9 on workflow structure and local execution because the pull-request gates had never run on this long-lived feature branch (they trigger on `pull_request`). PR #6's first full gate executions then exposed five latent defects, remediated in commits `9312b21`, `ea60ebb`, and the amendment-#16 follow-up, and recorded in `CHANGELOG.md`: (1) `python.yml` lint job missing `hypothesis` (mypy `untyped-decorator` on `strategies.py`), (2) `test_npu` hardcoding `/fake/cache` (uncreatable at the Linux filesystem root — ubuntu matrix `PermissionError`), (3) `parity.yml` missing the venv-creation step its `maturin develop` step requires, (4) `parity.yml` invoking bare `pytest`, which — unlike `python -m pytest` — does not place the repository root on `sys.path`, breaking the `parity.generate_corpus` import (reproduced and verified locally), and (5) cross-platform PRINet/torch reduction noise (up to ~1.27e-9 relative) on the derived corpus metric arrays exceeding the former `rtol=1e-10` METRIC tolerance on Linux regeneration of the Windows-authored corpus — remediated via maintainer-approved plan amendment #16 (METRIC `rtol` 1e-10 → 1e-8 for cross-platform corpus-regeneration comparisons; trajectories unchanged; single-runtime verification still targets 1e-10; Parity Report EA-002 register entry). All other 16-gate checks passed on first execution, including the Snyk SCA recipe (E-F5 interim gate) and Secret Scan.
