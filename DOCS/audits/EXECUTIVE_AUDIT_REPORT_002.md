...
- **Post-audit CI note (PR #6):** the executive audit verified E9 on workflow structure and local execution because the pull-request gates had never run on this long-lived feature branch (they trigger on `pull_request`). PR #6's first full gate executions then exposed four latent defects, remediated in commits `9312b21` and its parity-invocation follow-up, and recorded in `CHANGELOG.md`: (1) `python.yml` lint job missing `hypothesis` (mypy `untyped-decorator` on `strategies.py`), (2) `test_npu` hardcoding `/fake/cache` (uncreatable at the Linux filesystem root — ubuntu matrix `PermissionError`), (3) `parity.yml` missing the venv-creation step its `maturin develop` step requires, and (4) `parity.yml` invoking bare `pytest`, which — unlike `python -m pytest` — does not place the repository root on `sys.path`, breaking the `parity.generate_corpus` import (reproduced and verified locally). All other 16-gate checks passed on first execution, including the Snyk SCA recipe (E-F5 interim gate) and Secret Scan.
