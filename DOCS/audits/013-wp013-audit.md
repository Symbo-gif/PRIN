...
| A3 | Tests in tandem + coverage | ✅ | 36 new Rust tests — 16 in `bands.rs` (27 → 43 `#[test]` items), 2 in `temporal.rs` (36 → 38), 12 in `parity_bands.rs`, 6 in `parity_temporal.rs` — and 8 new Python acceptance tests (36 → 44), each committed with the code it covers. Workspace totals rose to 501 (504 with `strict-checks`). `bands.rs` 98.97%/98.68%, `temporal.rs` 99.79%/100%. |
