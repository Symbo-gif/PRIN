...
| WP036C-F7 | **FIXED.** `benchmarks/results/y4q1_9_preregistration_hash.json` removed. Root cause: `test_acceptance_y4q1_9.py::test_preregistration_runs` (a ported reference test) calls `bench_preregistration()` which `_save()`s the file into the tracked, currently-empty `benchmarks/results/` on every run — a bare `git clean` would not keep the tree clean. `.gitignore` now excludes `benchmarks/results/*_preregistration_hash.json`; real campaign artefacts are added deliberately at WP-038. | `807bb68` | `git status` clean after a full `pytest tests/` run; `git log --all --oneline -- benchmarks/results/y4q1_9_preregistration_hash.json` empty. Note: its removal correctly flipped `y4q2::TestArtefactCompleteness::test_has_json_files` from pass→fail (repo genuinely has 0 benchmark JSON) → now DV-031(A). |
