...
|---|---|---|---|
| WP017-F1 | FIXED | S3 commit — `fix(WP-017): F1 — buffer-pool size-validation gap`: added `MeanFieldRk4Error::PoolSizeMismatch { capacity, actual }` variant; added `CubeclBufferPool::capacity()` accessor (mirrors `MeanFieldRk4Buffers::capacity()`); added explicit `pool.capacity() != n` guard at the top of `step_cubecl_with_pool` (returns typed `Err` before any `array_arg` call); added symmetric guard in `step_cpu_with_pool`; updated `array_arg` SAFETY comment to document the upstream invariant; regression tests `step_cpu_with_pool_rejects_size_mismatch` (CPU pool, n=4 vs n=64) and `cubecl_pool_rejects_size_mismatch` (CubeCL CpuRuntime pool, n=4 vs n=4096) both assert `Err(PoolSizeMismatch { capacity: 4, actual: <call_n> })` | `cargo test -p prin-kernels --features cpu` — 46 tests pass including both new regression tests; `cargo test -p prin-kernels --features wgpu -- --test-threads=1` — 48 tests pass; `cargo clippy -p prin-kernels --all-targets --features cpu,wgpu -- -D warnings` clean |
| WP017-F2 | FIXED | S3 commit — same as F1: removed four dead `CubeclBufferPool` accessor methods (`k1_phase`, `out_phase`, `out_amp`, `out_freq` — zero call sites); added `CubeclBufferPool::capacity()` (tested); added `harness_default_impl` test (covers `Default` impl); added `verify_detects_genuine_backend_mismatch` test (exercises amplitude and frequency mismatch `?`-propagation arms through `verify_against_reference`); added `verify_with_tolerance_propagates_backend_error` test (exercises backend `Err` propagation through `verify_against_reference_with_tolerance`) | `cargo llvm-cov -p prin-kernels --features cpu --summary-only`: `buffers.rs` 100% lines / 100% functions (was 91.43%/71.43%); `equivalence.rs` 96.69% lines / 89.47% functions (was 94.61%/84.38%); both clear ≥95% line gate. `mean_field_rk4.rs` 99.56%, `backend.rs` 100%. `cubecl.rs` 77.59% — unchanged, consistent with amendment #10 kernel-body carve-out |
| WP017-F3 | FIXED | S3 commit — same as F1: implemented `step_auto` dispatcher in `cubecl.rs` that tries each backend in `auto_detect_order` priority (wgpu → cuda → CubeCL CPU) via the existing `try_step_*` functions, catching `BackendUnavailable` and falling through; final fallback to native `step_cpu` (always available). Regression test `step_auto_falls_back_to_available_backend` proves end-to-end fallback produces results matching the CPU reference at N=32. Dtype dispatch (f64) is a distinct feature beyond this WP's remediation scope; the `step_auto` entry point provides the dispatch architecture that dtype selection can extend in a future WP | `cargo test -p prin-kernels --features cpu` — `step_auto_falls_back_to_available_backend` passes; `cargo clippy -p prin-kernels --all-targets --features cpu,wgpu -- -D warnings` clean |
| WP017-F4 | AMENDED | S3 commit typed `fix(WP-017)` (not `docs`); the original S1 commit `4e507bc` retains its historical `docs(WP-017)` type (no history rewrite per WP015-F2 precedent) | Audit closure table records correct type; S3 commit uses correct Conventional Commits type |
| WP017-F5 | FIXED | S3 commit — `SESSION_REGISTER.md` row 0065 updated from `PLANNED` to `COMPLETE` | `grep "0065" SESSION_REGISTER.md` confirms `COMPLETE` |

**Delta re-audit date:** 2026-08-15 **Result:** CLEAN — all five findings closed; no new deviations introduced; all quality gates green.
