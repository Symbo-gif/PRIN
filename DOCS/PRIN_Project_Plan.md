...
2. **Tolerances:** trajectories `rtol=1e-6, atol=1e-8` (float64 reference); chaotic regimes compared statistically beyond the shadowing horizon; metrics/decompositions `rtol=1e-8` for cross-platform corpus-regeneration comparisons (amendment #16), with single-runtime metric/decomposition verification still targeting `rtol=1e-10` where achievable (WP-010/WP-014 acceptance).
