# PRIN Project State Report — Cycle 028

**Date:** 2026-08-22
**Cycle:** 028 (WP-028 "ONNX controller and backend selection")
**Completed sessions:** 0109–0112
...
- **Roadmap phase:** 5 — Daemon and experiment tooling (**1 of 5**
  phase-5 WPs complete: WP-028).
- **This cycle delivered:**
  - **Controller state/control types** (`crates/prin-daemon/src/state.rs`):
    `SubconsciousState`, `ControlSignals`, `Regime`, `STATE_DIM=32`,
    `CONTROL_DIM=8` — PRINet 3.0's exact float32 packing, normalisation, and
    clamping semantics (bit-exact parity).
  - **Backend detection and selection** (`crates/prin-daemon/src/backend.rs`):
    `Backend`, `BackendSelection`, `SelectionReason`, `select_backend`,
    `provider_options`, `VitisAiConfig`, `firmware_candidates`,
    `resolve_firmware` — VitisAI→DirectML→CPU priority policy and the
    deterministic CPU-terminated fallback ladder (pure, property-tested).
  - **Hand-written ONNX model reader** (`crates/prin-daemon/src/onnx.rs`):
    `OnnxModelInfo`, `TensorSpec`, `inspect_onnx_bytes`, `inspect_onnx_file`
    — bounded, total `ModelProto` decoder over eleven message fields, no
    `protoc` dependency, proptest-fuzzed on arbitrary bytes (never panics).
  - **SHA-256 model validation** (`crates/prin-daemon/src/model.rs`):
    `ModelManifest`, `sha256_file`, `verify_sha256`,
    `ControllerModel::validate`, `validate_controller_contract` — integrity
    verification against `models/manifest.json`, external-data completeness,
    and graph-contract validation.
  - **Typed error hierarchy** (`crates/prin-daemon/src/error.rs`):
    `DaemonError` — 18 typed variants; no `panic!`/`unwrap`/`expect` in
    library code.
  - **PyO3 bindings** (`crates/prin-py/src/bindings/daemon.rs`): 14
    `#[pyfunction]`/`#[pyclass]` exports plus eight module constants.
  - **Python orchestration layer** (`python/prin/daemon.py`):
    `SubconsciousController`, `create_session`, `select_backend`,
    `detect_best_backend`, `backend_info`, `verify_model_artefacts`,
    `OrtUnavailableError` — no numerics; ONNX Runtime session creation stays
    in Python per risk register #4.
  - **Model manifest** (`models/manifest.json`, `models/README.md`): SHA-256
    + size manifest for the two committed controller artefacts.
  - **S2 audit** (`DOCS/audits/028-wp028-audit.md`): verdict `PASS`, zero
  ...
  - **S4 documentation:** updated `crates/prin-daemon/README.md`,
    `crates/prin-py/README.md`, `crates/README.md`, `CHANGELOG.md`,
    `DOCS/sphinx/migration_guide.rst`, `DOCS/sphinx/api/daemon.rst` (new),
    `DOCS/baselines/wp001_api_traceability.md`,
    `DOCS/sessions/SESSION_REGISTER.md`, `DOCS/sessions/phase-5/README.md`,
    `DOCS/reports/README.md`; wrote this report; declared WP-029.
- **Plan conformance:** ON TRAJECTORY WITH AMENDMENTS — amendments #1–#30
  remain in force; no new amendment was required this cycle.
- **Audit:** `DOCS/audits/028-wp028-audit.md` — S2 verdict `PASS`, zero
  findings. S3 no-change closure with CLEAN delta re-audit. No unresolved
  D1/D2 finding exists.
- **Session Register:** 0109 (S1), 0110 (S2), 0111 (S3), 0112 (S4) all
  marked **COMPLETE**; 0113 (WP-029 S1) is the registered successor.

---

## 2. Metric trends

| Metric | Previous (PSR-027) | Current (PSR-028) | Gate |
|---|---|---|---|
| Rust tests passing | 1144/1144 default workspace; `prin-daemon` new crate with 122 tests | **1266/1266** default workspace, 0 failed, 1 ignored; `prin-daemon` **122** (81 lib + 7 integration + 8 parity + 11 property + 15 doctest) | 100% where defined |
| Python tests passing | 441 passed, 8 deselected (fast suite); full suite 959 passed | **547 passed**, 8 deselected (fast suite); full suite `pytest tests/ parity/` **1147 passed**, 0 failed | 100% |
| Coverage (changed code) | `dataset.rs` 99.75%/99.56%/100%; `losses.rs` 100%/97.71%/100%; `trainer.rs` 96.26%/97.35%/100% | `backend.rs` **99.50%**/100%/100%; `model.rs` **96.52%**/97.78%/99.53%; `onnx.rs` **95.16%**/95.45%/99.38%; `state.rs` **99.05%**/100%/100%; `python/prin/daemon.py` **100%** (204/204) — all ≥95% | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (231/231) | 100% public (**262/262**) — `daemon.py` 100%; Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` | ≥95% overall, 100% public |
| Parity cases passing / total defined | 510 parity-marked Python tests; 15 `prin-train` golden-value parity tests | 510 parity-marked Python tests (unchanged); **15** `prin-train` golden-value parity tests (unchanged); **+8** `prin-daemon` Rust bit-exact parity tests; **+82** differential parity tests (`parity/test_parity_subconscious.py`) | 100% at tolerance |
...
| Sphinx warning-as-error build | 0 warnings | 0 warnings (fresh-directory build; `api/daemon.rst` and `migration_guide.rst` updated with WP-028 symbol mappings) | 0 warnings |
...
| Benchmark regression gates | none defined for `prin-train`; none tripped | none defined for `prin-daemon`; none tripped | none tripped |

**Verification commands re-run in S4 (2026-08-22, independent of S2/S3 evidence):**
...
`DOCS/reports/027-project-state.md` §3 unchanged (verified by
...
| WP016-F4 | 016 | D2 | No N=1M `OscilloSim` parity/scale evidence | FIXED | `35dbb3e`; N=100k determinism/memory regression test + N=1M benchmark evidence `[RETROACTIVE UPDATE - Executive Audit 004]` |
...
- **DV-019 (gradient-flow test flake):** Did not recur in any WP-028
...
- **DV-021 (bridge overhead `<5%` gap):** WP-028 touches no bridge code.
...
- **DV-024 (self-hosted runner offline):** Discovered at Phase 4
  recommendation session. Status: OPEN — unrelated to WP-028.
- **DV-010 (Phase 1/2 pre-release tag):** Version logic resolved; tag push
  pending explicit maintainer action. Status: OPEN.

---

## 6. Next work package declaration — WP-029

- **Title:** Daemon runtime and lock-free control buffer
- **Scope (files/crates/modules):** Native daemon lifecycle, bounded
  control-signal ring buffer, telemetry, shutdown, and concurrency safety.
- **Plan sections advanced:** §6 (Phase 5 roadmap), §5 (architecture rules).
- **Acceptance criteria:** Stress/race/lifecycle tests pass; no
  deadlocks/data races; p50/p95 instrumentation is valid and latency pilot
  improves on 3.0.
- **Non-goals:** Training hooks or MOT evaluation.
- **First session brief:** `DOCS/sessions/phase-5/0113-wp029-s1-daemon-runtime-and-lock-free-control-buffer.md`
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-029 S1 begins)
...
(WP-028 is the first of five Phase 5 WPs), so item 9's explicit verification
requirement does not apply this cycle. The three documents it names were last
verified at PSR-027 (Phase 4 close):
