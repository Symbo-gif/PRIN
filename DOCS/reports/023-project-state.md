# PRIN Project State Report — Cycle 023

**Date:** 2026-08-18  
**Cycle:** 023 (WP-023 "Inhibition, activations, and HEP")  
**Completed sessions:** 0089–0092  
**Author:** Devin (AI pair), approved by maintainer  
**Git state:** `main` @ `d64d7fe` (S3 brief completion); S4 documentation closure follows  

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 4 — Trainable stack and Torch bridge (**2 of 6** phase-4 WPs
  complete: WP-022, WP-023; WP-024 through WP-027 remain).
- **This cycle delivered:**
  - **Second `prin-train` implementation increment** (`crates/prin-train/`), extending
    the Burn-based trainable stack:
    - `inhibition::FeedbackInhibition` — feedback lateral inhibition with a
      straight-through estimator (STE) for competitive $k$-WTA activation (rebuild of
      the trainable half of PRINet 3.0 `core.propagation.inhibition.FeedbackInhibition`):
      hard top-$k$ selection in the forward pass and soft temperature-scaled sigmoid
      backward gradients, with optional target-sparsity parameterization
      ($k = \lfloor (1 - s) \cdot N \rfloor$).
    ...
    - `hep::HolomorphicEp` — Holomorphic Equilibrium Propagation trainer (rebuild
      ...
      differences ($\frac{1}{2\beta} (z_{+\beta} z_{+\beta}^\dagger - z_{-\beta} z_{-\beta}^\dagger)$).
    - `error::TrainError` — extended with four new typed variants (`InvalidK`,
      `DivisionByZero`, `StepLimitExceeded`, `BetaTooSmall`), all re-exported at the
      crate root.
    - **93 unit/property tests** (S1) + 1 compile-time regression test (S3) + 5 new
      golden-value parity tests against PRINet 3.0 (`parity_inhibition.rs` 1,
      `parity_activations.rs` 2, `parity_energy.rs` 2) + 6 doctests across the
      four new modules, with line coverage ranging from **98.61% to 99.59%** and
      **100% function coverage** across all source files.
  - **S2 audit** (`DOCS/audits/023-wp023-audit.md`): verdict `PASS-WITH-FINDINGS`,
    one D4 finding (WP023-F1: `public_api.rs` regression test did not cover
    `GatedPhaseActivationParams` crate-root re-export).
  - **S3 remediation:** WP023-F1 FIXED (commit `11821c0`, extended `public_api.rs`
    with `accepts_gated_phase_activation_params` compile-guard). CLEAN delta re-audit
    appended to the Audit Report.
  - **S4 documentation:** updated `crates/prin-train/README.md`, `crates/README.md`,
    `CHANGELOG.md`, `DOCS/sphinx/migration_guide.rst` (WP-023 symbol entries),
    ...
    wrote this report; declared WP-024.
- **Plan conformance:** ON TRAJECTORY WITH AMENDMENTS — amendments #1–#27 remain in
  force; no new plan amendment was required this cycle.
- **Audit:** `DOCS/audits/023-wp023-audit.md` — S2 verdict `PASS-WITH-FINDINGS`,
  one D4 finding, closed in S3 (FIXED); CLEAN delta re-audit. No unresolved D1/D2
  finding exists.
- **Session Register:** 0089 (S1), 0090 (S2), 0091 (S3), 0092 (S4) all marked
  **COMPLETE**; 0093 (WP-024 S1) is the registered successor.
- **S4 consistency-sweep corrections:** at S4 entry, the `SESSION_REGISTER.md` and
  `DOCS/sessions/phase-4/README.md` row for session 0091 still read `PLANNED` while
  its brief was committed as `COMPLETE` — corrected in this session's commit from
  committed evidence per Documentation Standards §7 item 6; `tools/wp001_baseline.py check`
  passes clean.
...
| Metric | Previous (PSR-022) | Current (PSR-023) | Gate |
|---|---|---|---|
| Rust tests passing | 859/859 default workspace (829 unit/integration/property + 30 doctests); `prin-train` default 36 (33 unit + 2 parity + 1 `public_api`) + 2 doctests = 38; `prin-train --features strict-checks` 38 (35 unit + 2 parity + 1 `public_api`) + 2 doctests = 40 | **931/931** default workspace (897 unit/integration/property + **34 doctests**), 0 failed; `prin-train` default 101 (93 unit + 7 parity + 1 `public_api`) + 6 doctests = 107; `prin-train --features strict-checks` 103 (95 unit incl. 2 NaN-guard + 7 parity + 1 `public_api`) + 6 doctests = 109 | 100% where defined |
...
| Coverage (changed code) | `crates/prin-train/src/bands.rs` 99.26% lines / 100% functions; `layers.rs` 99.32% lines / 100% functions; `support.rs` 98.31% lines | `crates/prin-train/src/activations.rs` **99.40%** lines (331/333) / 100% functions (43/43); `energy.rs` **98.61%** lines (213/216) / 100% functions (18/18); `hep.rs` **98.92%** lines (276/279) / 100% functions (21/21); `inhibition.rs` **99.59%** lines (244/245) / 100% functions (29/29); `bands.rs` 99.26% lines / 100% functions; `layers.rs` 99.32% lines / 100% functions; `support.rs` 98.46% lines (64/65) / 90.00% functions (9/10 — the 1 uncovered function is the `#[cfg(not(feature = "strict-checks"))]` no-op stub) | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (106/106) — unchanged; Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` | 100% public (106/106) — unchanged, no Python files touched; Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` (100% public-item rustdoc across all crates) | ≥95% overall, 100% public |
| Parity cases passing / total defined | 510 parity-marked Python tests; +2 `prin-train` golden-value parity tests vs. PRINet 3.0 | 510 parity-marked Python tests (unchanged); **+5 new `prin-train` golden-value parity tests** vs. PRINet 3.0 (`parity_inhibition.rs` 1 at `rtol=1e-10, atol=1e-12`, `parity_activations.rs` 2 at `rtol=1e-6`, `parity_energy.rs` 2 at `rtol=1e-10, atol=1e-12`), bringing total `prin-train` parity tests to **7**, all passing; closed-form HEP coupling gradients cross-checked two ways (autodiff `<1e-8`, finite difference `<1e-3`, manual outer-product `<1e-9`) | 100% at tolerance |
...
| Sphinx warning-as-error build | 0 warnings | 0 warnings (`migration_guide.rst` extended with the WP-023 entries, rebuilt clean under `-W --keep-going`) | 0 warnings |
| Snyk Code / Snyk Open Source | Snyk Code: CI authoritative gate; local scan blocked (unauthenticated). Snyk Open Source: N/A for Cargo, `cargo audit` authoritative | Snyk Code: local Snyk re-scan BLOCKED — Snyk MCP reports unauthenticated on this machine (reported as blocked per Coding Standards §6; not claimed as passed; CI remains the authoritative gate). Snyk Open Source: N/A for Cargo (unsupported package manager, R23/`SNYK-CLI-0008`); `cargo audit` is the authoritative ecosystem-native gate and is clean at the governed threshold | 0 at gate threshold |
...
cargo clippy --workspace --all-targets --features prin-train/strict-checks -- -D warnings # clean (exit 0)
$env:RUSTDOCFLAGS='-D warnings'; cargo doc --workspace --no-deps        # 0 warnings
cargo test --workspace                                                  # 931 passed, 0 failed (incl. 34 doctests)
cargo test -p prin-train                                                # 101 + 6 doctests passed
cargo test -p prin-train --features strict-checks                       # 103 + 6 doctests passed
cargo llvm-cov -p prin-train --features strict-checks                   # activations 99.40%, energy 98.61%, hep 98.92%, inhibition 99.59%, bands 99.26%, layers 99.32%, support 98.46% lines
...
.venv\Scripts\python tools/check_deviation_ledger.py DOCS/reports/022-project-state.md DOCS/reports/023-project-state.md   # ledger consistency check passed
gh api repos/Symbo-gif/PRIN --jq '{secret_scanning: .security_and_analysis.secret_scanning.status, push_protection: .security_and_analysis.secret_scanning_push_protection.status}' # DV-009 re-check: both null (unavailable on this private repo), amendment #5 substitute remains in force
```
...
One new finding was raised and closed this cycle: WP023-F1 (D4, `public_api.rs`
regression test did not cover `GatedPhaseActivationParams` crate-root re-export),
FIXED in S3 commit `11821c0`. The cumulative table below carries forward all rows
from `DOCS/reports/022-project-state.md` §3 unchanged (verified by
`tools/check_deviation_ledger.py`, run in two-report mode) with the new WP-023
row appended.
...
Amendments #1–#27 remain in force. No new plan amendments were required this cycle.
...
- **DV-005 (CUDA DLPack validation):** re-audited at WP-022/WP-023 — unaffected
...
- **DV-008 (`paste` advisory):** re-checked with `cargo audit` this cycle (2026-08-18):
...
- **DV-010 (pre-release tags):** `v0.3.0-alpha.1` covers Phase 1/2; `v0.4.0-alpha.1` prepared
  at Phase 3 exit. Tag push remains deferred to explicit maintainer release action.
...
- **DV-019 (WP-022 `bands::tests::gradients_flow_to_every_parameter` thread contention):**
  ...
  test module. Tracked for future investigation.
- **Snyk local scanning unavailable (standing observation):** Snyk MCP reports unauthenticated
  on this machine as of 2026-08-18, so the per-cycle local Snyk Code re-scan could not be executed
  and is reported as **blocked** (never claimed as passed), per Coding Standards §6. The authoritative
  CI `snyk` workflow is the merge gate.

---

## 6. Trajectory verdict

**ON TRAJECTORY.** Phase 4 is 2 of 6 WPs complete (WP-022, WP-023). The cycle delivered
the declared scope in full — feedback inhibition with STE (`FeedbackInhibition`), complex
and periodic phase activations (`d_silu`, `ComplexTensor`, `HolomorphicActivation`,
`phase_activation`, `GatedPhaseActivation`), holomorphic energy functions
(`HolomorphicEnergy`), and Equilibrium Propagation training (`HolomorphicEp`) — with closed-form
gradient verification, finite-difference and autodiff cross-checks, energy decomposition
properties, STE identity tests, and golden-value parity tests against PRINet 3.0 all covered
and green; the contract's non-goals (optimizers, Python bridge, full training loops) were
untouched. One D4 audit finding (WP023-F1) was closed in S3 (FIXED) with a CLEAN delta re-audit.
No unresolved D1/D2 finding exists and no finding is carried. The successor WP-024 (Oscillator-aware
optimizers, Phase 4, sessions 0093–0096) is declared below.

---

## 7. Next work package declaration — WP-024

- **Title:** WP-024 — Oscillator-aware optimizers
- **Scope (files/crates/modules):**
  - `crates/prin-train/src/optimizers.rs` (or modular `optimizer` submodules)
  - `crates/prin-train/src/phase_adam.rs` (Riemannian phase-aware Adam optimizer)
  - `crates/prin-train/src/kuramoto_optimizer.rs` (Kuramoto order-parameter-aware natural gradient / adaptive learning rate optimizer)
  - `crates/prin-train/src/error.rs` (optimizer-specific `TrainError` variants if needed)
  - `crates/prin-train/src/lib.rs` (module exports and public API re-exports)
  - `crates/prin-train/tests/parity_optimizers.rs` (golden-value parity against PRINet 3.0 `nn/optimizers.py`)
- **Plan sections advanced:** Project Plan §6 (Phase 4), §8.1 (WP-024), Target Architecture §5 (`prin-train`).
- **Acceptance criteria:**
  - `PhaseAdam` implementation supporting Riemannian phase angle updates ($2\pi$-modular geodesic step) with momentum and second-moment tracking.
  - `KuramotoOptimizer` implementing order-parameter-scaled learning rate adaptation based on network coherence.
  - Step invariants, parameter validity checks, deterministic seed flow, and serialization support.
  - Golden-value parity tests against PRINet 3.0 `nn/optimizers.py` (`PhaseAdam`, `KuramotoOptimizer`) in `tests/parity_optimizers.rs`.
  - ≥95% line coverage on new/changed code, 100% public docstrings, 0 clippy/fmt/audit/test failures.
- **Non-goals:** Python bindings / `torch.optim` wrapper (WP-025) or full training loop integration (WP-027).
- **First session brief:** `DOCS/sessions/phase-4/0093-wp024-s1-oscillator-aware-optimizers.md`
- **Maintainer approval:** Approved by maintainer, 2026-08-18.
