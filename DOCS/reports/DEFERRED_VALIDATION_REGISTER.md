...
| DV-004 | **`cargo-llvm-cov` kernel body coverage gap.** `#[cube(launch)]` kernel bodies are non-instrumentable; reported line coverage understates actual test coverage. Kernel equivalence validates correctness. Re-audited at WP-017, WP-018, and WP-019: the `#[cube(launch)]` stubs (now six — three from `mean_field_rk4/cubecl.rs`, two from `sparse_knn/cubecl.rs` (`sparse_knn_coupling`), and one from `pac/cubecl.rs` (`pac_phase_sum_block_reduce`, `pac_modulate`)) remain non-instrumentable; instrumentable surrounding code is ≥95% (`sparse_knn/cubecl.rs` raw 85.86%, `pac/cubecl.rs` raw 82.93% under `--features wgpu,cpu`, both at or above the pre-existing `mean_field_rk4/cubecl.rs` baseline of 80.76%). | WP-004 (WP004-F2) | Plan amendment #10 | Phase 3 (WP-020) — remains open while more `#[cube(launch)]` kernels are added | OPEN — six non-instrumentable kernel bodies now present (up from three at WP-018); instrumentable surrounding code ≥95% |
