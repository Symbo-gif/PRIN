...
| DV-003 | **Device-event kernel timing.** `StepReport.wall_time_seconds` uses host wall-clock, not device events. Documented as prototype caveat. | WP-004 (WP004-F8) | — (documented caveat) | Phase 3 (WP-017) — re-audited; no action required in WP-017 per non-goal "Production GPU kernels" | OPEN — device-event timing remains a future Phase 3 WP optimization (WP-018/019/020/021) |
| DV-004 | **`cargo-llvm-cov` kernel body coverage gap.** `#[cube(launch)]` kernel bodies are non-instrumentable; reported line coverage understates actual test coverage. Kernel equivalence validates correctness. Re-audited at WP-017: the mean-field RK4 `cubecl.rs` `#[cube(launch)]` stubs remain non-instrumentable; instrumentable surrounding code is ≥95%. | WP-004 (WP004-F2) | Plan amendment #10 | Phase 3 (WP-017) — re-audited; remains open while more `#[cube(launch)]` kernels are added | OPEN — non-instrumentable kernel bodies still present; instrumentable surrounding code ≥95% |
