...
| DV-004 | **`cargo-llvm-cov` kernel body coverage gap.** `#[cube(launch)]` kernel bodies are non-instrumentable; reported line coverage understates actual test coverage. Kernel equivalence validates correctness. Re-audited at WP-017, WP-018, WP-019, and WP-020: the `#[cube(launch)]` stubs (now ten — three from `mean_field_rk4/cubecl.rs`, two from `sparse_knn/cubecl.rs` (`sparse_knn_coupling`), one from `pac/cubecl.rs` (`pac_phase_sum_block_reduce`, `pac_modulate`), and four from `discrete_step/cubecl.rs` (`complex_order_reduce`, `real_sum_reduce`, `band_euler_step`, `pac_gate`)) remain non-instrumentable; instrumentable surrounding code is ≥95% (`discrete_step.rs` 97.65%, `discrete_step/cubecl.rs` raw 79.32% under `--features wgpu,cpu` — 1.44 points below the `mean_field_rk4/cubecl.rs` baseline of 80.76%, fully explained by the higher kernel-body-to-total-code ratio (4 vs. 3), not a coverage regression in reachable code). | WP-004 (WP004-F2) | Plan amendment #10 | Phase 3 (WP-021) — remains open while more `#[cube(launch)]` kernels are added | OPEN — ten non-instrumentable kernel bodies now present (up from six at WP-019); instrumentable surrounding code ≥95% |
...
| DV-012 | **`prin-py` sweep/engine PyO3 bindings and `prin-kernels` CPU-reference work.** Original WP-016 declared scope, narrowed out by amendment #20. `prin-kernels` half closed by WP-017 (sessions 0065/0066/0067, audit `DOCS/audits/017-wp017-audit.md`): `step_cpu`/`step_cpu_with_pool` are the authoritative CPU references; `step_auto` provides device dispatch; backend abstraction, buffer pooling, and equivalence harness are operational. S4 documentation reviewed and confirmed closure. `prin-py` sweep/engine bindings remain open and are assigned to a future WP by the Phase 3 exit-gate PSR at latest (R19). | WP-016; amendment #20 | `prin-kernels`: WP-017 (audit 017, 2026-08-15). `prin-py`: at latest the Phase 3 exit-gate PSR (WP-021 S4, session 0084) | CLOSED (`prin-kernels` half); OPEN (`prin-py` half) — `prin-kernels` half closed by WP-017 S3 remediation and S4 confirmation; `prin-py` half deferred per R19 |
...
|---|---|---|---|
| R19 | Assign a WP/phase to the deferred `prin-py`/`prin-kernels` carried scope | Phase 3 exit-gate PSR (WP-021 S4, session 0084) at latest | `prin-kernels` half assigned to WP-017 and closed. `prin-py` sweep/engine bindings remain open and are assigned to a future Phase 3 WP, with the final assignment recorded by the Phase 3 exit-gate PSR at latest (maintainer approval required). |
