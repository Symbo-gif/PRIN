...
| DV-012 | **`prin-py` sweep/engine PyO3 bindings and `prin-kernels` CPU-reference work.** Original WP-016 declared scope, narrowed out by amendment #20; no successor WP yet assigned. Not a Phase 3 blocker but should land before Phase 7 scientific campaigns use the Python sweep API. **`prin-kernels` half CLOSED by WP-017** (sessions 0065/0066/0067, audit `DOCS/audits/017-wp017-audit.md`): `step_cpu`/`step_cpu_with_pool` are the authoritative CPU references; `step_auto` provides device dispatch; backend abstraction, buffer pooling, and equivalence harness are operational. `prin-py` sweep/engine bindings remain OPEN. | WP-016; amendment #20 | `prin-kernels`: WP-017 (audit 017, 2026-08-15). `prin-py`: at latest the Phase 3 exit-gate PSR (WP-021 S4, session 0084) | OPEN (`prin-py` half only) — `prin-kernels` half closed by WP-017 S3 remediation (2026-08-15) |
