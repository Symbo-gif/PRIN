...
# PRIN Project State Report — Cycle 026

**Date:** 2026-08-19
**Cycle:** 026 (WP-026 "PhaseTracker, Hybrid, baselines, and allocation")
**Completed sessions:** 0101–0104 (plus Exec-WP-026 S1 executive secondary session)
**Author:** Qwen Code (AI pair), maintainer-reviewed
**Git state:** `main` @ `2135077` (S3 closure); S4 documentation closure follows

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 4 — Trainable stack and Torch bridge (**5 of 6** phase-4
  WPs complete: WP-022, WP-023, WP-024, WP-025, WP-026; WP-027 remains).
- **This cycle delivered:**
  - **Six new trainable modules** in `crates/prin-train/` rebuilding PRINet 3.0
    `nn/{layers,hybrid,slot_attention,ablation_variants,adaptive_allocation}.py`:
    - `attention::OscillatoryAttention` — multi-head attention with additive
      oscillatory coherence bias (`score = QKᵀ/√d_k + α·cos(φ_i − φ_j)`).
    - `phase_tracker::PhaseTracker` — PRIN's primary contribution: phase-based
      multi-object tracker encoding detections to phase/amplitude via MLP,
      evolving through `DiscreteDeltaThetaGamma`, matching frames by
      phase-coherence similarity with greedy assignment.
    - `hybrid::HybridPRINetV2` — canonical hybrid oscillator + attention
      classification architecture.
    - `slot_attention::{SlotAttentionModule, TemporalSlotAttentionMOT}` —
      non-oscillatory Slot Attention comparison baseline.
    - `ablation::{PhaseTrackerFrozen, PhaseTrackerStatic, SlotAttentionNoGRU,
      SlotAttentionFrozen}` — four structural ablation variants.
    - `allocation::{AdaptiveOscillatorAllocator, DynamicPhaseTracker}` —
      complexity-driven adaptive oscillator-count allocation.
  ...
  - **PyO3 bridges and Python wrappers** (Exec-WP-026 S1 executive secondary
    session): six new bridge modules in `crates/prin-py/src/bindings/`,
    thin `torch.nn.Module` wrappers in `python/prin/nn/`, generic
    `apply_rust_bridge` in `_bridge.py`, 344 lines of new `.pyi` stubs.
    `validate_shapes()` added to all six new Module types, finding and fixing
    a real checkpoint-corruption bug (Burn's `Module::load_record` silently
    ignores plain `usize` fields).
  - **S2 audit** (`DOCS/audits/026-wp026-audit.md`): verdict
    `PASS-WITH-FINDINGS`, two D4 findings: WP026-F1, WP026-F2.
  - **S3 remediation:** WP026-F1 FIXED (strategy-mismatch detection in
    ...
    variant, 2 regression tests); WP026-F2 FIXED (whole-module
    `HybridPRINetV2` parity test via `HybridPRINetV2Params`/`init_from_params`,
    revealed and fixed missing ReLU in classifier head). CLEAN delta re-audit.
  - **S4 documentation:** updated `crates/prin-train/README.md`,
    ...
    `DOCS/reports/README.md`, `DOCS/README.md`; wrote this report; declared
    WP-027.
- **Plan conformance:** ON TRAJECTORY WITH AMENDMENTS — amendments #1–#29
  remain in force; no new amendment was required this cycle.
- **Audit:** `DOCS/audits/026-wp026-audit.md` — S2 verdict
  `PASS-WITH-FINDINGS`, two D4 findings, both closed in S3 (FIXED); CLEAN
  delta re-audit. No unresolved D1/D2 finding exists.
- **Session Register:** 0101 (S1) + Exec-WP-026 S1 marked **COMPLETE**;
  0102 (S2), 0103 (S3), 0104 (S4) all marked **COMPLETE**; 0105 (WP-027 S1)
...
| Metric | Previous (PSR-025) | Current (PSR-026) | Gate |
|---|---|---|---|
| Rust tests passing | 998/998 default workspace; `prin-train` default 168 + 9 doctests = 177 | **1131/1131** default workspace, 0 failed; `prin-train` default **275** (lib) + **16** (integration: 14 parity + 2 `public_api`) + **13** doctests = **304** | 100% where defined |
| Python tests passing | 335 passed, 8 deselected (fast suite); full suite 853 passed | **421 passed**, 8 deselected (fast suite); full suite `pytest tests/ parity/` **939 passed**, 0 failed | 100% |
| Coverage (changed code) | `layers.rs` 97.81%/99.35%; `activations.rs` 98.93%/99.45%; `nn/__init__.py` 100% | `ablation.rs` **98.54%**/100%/99.60%; `allocation.rs` **98.14%**/97.22%/97.16%; `attention.rs` **97.40%**/100%/99.40%; `hybrid.rs` **98.10%**/100%/97.51%; `phase_tracker.rs` **96.01%**/97.50%/96.60%; `slot_attention.rs` **98.41%**/100%/98.90%; `support.rs` **99.05%**/100%/99.54%; `python/prin/nn/` **100%** (372/372 statements) | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (123/123) | 100% public (**213/213**) — `nn/` files 108/108; Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` | ≥95% overall, 100% public |
| Parity cases passing / total defined | 510 parity-marked Python tests; 12 `prin-train` golden-value parity tests | 510 parity-marked Python tests (unchanged); **15** `prin-train` golden-value parity tests (+3: `parity_attention`, `parity_phase_tracker`, `parity_hybrid`) | 100% at tolerance |
...
| Sphinx warning-as-error build | 0 warnings | 0 warnings (`migration_guide.rst` updated with WP-026 symbol mappings and bridge entries) | 0 warnings |
...
.venv\Scripts\python -m bandit -r . -c pyproject.toml                   # 0 issues (5725 lines)
...
.venv\Scripts\python tools/check_deviation_ledger.py DOCS/reports/025-project-state.md DOCS/reports/026-project-state.md   # ledger consistency check passed
```
...
Two new findings were raised and closed this cycle: WP026-F1 (D4, FIXED),
WP026-F2 (D4, FIXED). The cumulative table below carries forward all rows
from `DOCS/reports/025-project-state.md` §3 unchanged (verified by
`tools/check_deviation_ledger.py`, run in two-report mode) with the new
WP-026 rows appended.
...
- **DV-021 (unchanged):** boundary overhead at small shapes (+40.6%) exceeds
  the `<5%` acceptance target. The bridges are fully correct; this is a
  performance-target gap. WP-027 S1's own acceptance criterion ("bridge
  overhead <5%") and mission text ("bridge profiling") cover this.
- **DV-019 (unchanged):** `bands::tests::gradients_flow_to_every_parameter`
  ...
  record. Not triggered during this cycle's audit test runs.

---

## 6. Next work package declaration — WP-027

- **Title:** Trainable-stack integration and Phase 4 gate
- **Scope (files/crates/modules):** `crates/prin-train/` (integration of
  all trainable modules into a unified training pipeline); `crates/prin-py/`
  (bridge profiling, serialization acceptance); `python/prin/nn/` (trainable
  API acceptance, `torch.optim.Optimizer` wrapper over `SyncGd`/`Rip`/`Scalr`).
- **Plan sections advanced:** §6 (Phase 4 roadmap, Phase 4 gate), §5
  (architecture rules).
- **Acceptance criteria:** PhaseTracker reaches at least the registered 3.0
  IP threshold in validation runs; gradchecks green; bridge overhead <5%
  (DV-021 concrete checkpoint); Phase 4 tag gate passes without treating
  pilot data as campaign evidence. DV-005 (CUDA DLPack) re-audited at this
  Phase 4 gate — decide whether a CUDA Burn backend enters Phase 5 scope.
- **Non-goals:** Final scientific publication claims; confirmatory temporal
  CLEVR conclusions.
- **First session brief:** `DOCS/sessions/phase-4/0105-wp027-s1-trainable-stack-integration-and-phase-4-gate.md`
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-027 S1 begins)
