# PRIN Project State Report — Cycle 035

**Date:** 2026-08-27
**Cycle:** 035 (WP-035 "Reproduction pipeline and manifest")
**Completed sessions:** 0137–0140
**Author:** Qwen Code (AI pair), approved by maintainer MichaelMaillet
**Git state:** `main` @ S4 closure (predecessor `9c560b9`; this session's commit follows)

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 6 — Benchmarks, reproduction, docs, and RC1 (**3 of 6**
  phase-6 WPs complete: WP-033, WP-034, WP-035).
- **This cycle delivered:**
  - **`tools/reproduce.py`** (421 lines): complete deterministic reproduction
    pipeline and CLI. Regenerates all 14 verifiable historical figures
    ...
    random sampling. Typed error hierarchy (`ReproductionError`,
    `ReproductionConfigurationError`, `ManifestFormatError`,
    `ManifestMismatchError`), SHA-256 hashing, manifest load/append/verify,
    reproduction runner, and CLI with `--figures-only`/`--tables-only`/
    `--verify-manifest`/`--append-manifest` modes. Manifest destination
    confined to allowed roots; path-traversal and duplicate-name checks on
    manifest records.
  - **`paper/artefact_manifest.json`** (867 lines): governed SHA-256 manifest
    (schema version 1, 172 records) covering every stored JSON artefact.
    Append-only: existing records verified before new ones added; mutation or
    removal of an accepted artefact is a hard failure.
  - **`tests/test_reproduce.py`** (337 lines): 23 tests covering manifest
    validation (172 records), append-only semantics (add/modify/delete/resize),
    three tamper mutation modes (missing/corrupt/extra), size-before-hash
    optimization, 8 malformed-manifest parametric cases, output mode selection,
    ...
    import regression. `tools/reproduce.py` **100%** line coverage (161
    statements, 0 miss).
  - **`.github/workflows/repro.yml`** (12-line delta): `PRIN_REPRO_ENABLED`
    guard removed; runs tamper tests then verified regeneration on every
    push/PR.
  - **S1 handoff** (`DOCS/experiments/0137-wp035-s1-handoff.md`): full
  ...
  - **S2 audit** (`DOCS/audits/035-wp035-audit.md`): verdict
    `PASS-WITH-FINDINGS`, one D4 finding (WP035-F1).
  - **S3 remediation:** WP035-F1 FIXED (`cbbbbb3`); delta re-audit CLEAN.
  - **S4 documentation:** `tools/` and `paper/` READMEs expanded; Migration
    Guide gains a WP-035 section; `CHANGELOG.md` WP-035 entry; this report;
    WP-036 declared (§6).
...
- **Audit:** `DOCS/audits/035-wp035-audit.md` — S2 verdict
  `PASS-WITH-FINDINGS` (1 D4 finding). S3 FIXED WP035-F1, delta re-audit
  CLEAN. No unresolved D1/D2 finding exists.
- **Session Register:** 0137 (S1), 0138 (S2), 0139 (S3), 0140 (S4) all marked
  **COMPLETE**; 0141 (WP-036 S1) is the registered successor, present in
...
| Metric | Previous (PSR-034) | Current (PSR-035) | Gate |
|---|---|---|---|
| Rust tests passing | 1447 passed across 48 test binaries, 0 failed, 1 ignored | **Unchanged** — 1447 passed, 0 failed, 1 ignored (`cargo test --workspace --quiet`); WP-035 touches no `crates/` file, adds no Rust code, and changes no `Cargo.*` manifest | 100% where defined |
| Python tests passing | 696 passed, 9 deselected (fast suite); full suite 1297 passed | **719 passed**, 9 deselected (fast suite, `-m "not slow and not gpu"`); full suite (`tests/` + `parity/`) **1320 passed** (+23 from WP-035's `test_reproduce.py`) | 100% |
| Coverage (changed code) | `prin.reporting` 99% lines | `tools/reproduce.py` **100%** lines (161 stmts, 0 miss) | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 95.6% overall (351/367), PASS | **95.6%** overall (351/367), PASS — unchanged; WP-035 adds no `python/prin` symbols | ≥95% overall, 100% public |
| Parity cases passing / total defined | Unchanged from PSR-033 | Unchanged — WP-035 introduces **no numerical primitive**. Its applicable golden evidence is stored-artefact byte comparison (11 LaTeX fragments regenerate byte-identical) and deterministic PDF/PNG normalization (stable across repeated generation); 172 manifest records verify; 39 outputs generated (28 figure files + 11 tables); tamper tests fail closed on all three mutation modes | 100% at tolerance |
| Clippy/ruff/mypy/bandit/audit findings | 0 across all gates; `cargo audit` exit 0 with 2 allowed warnings | 0 across all gates; `cargo audit` exit 0 with **2 allowed warnings** (DV-008 `paste`, DV-017 `bincode` — unchanged); `pip-audit` clean (project + `DOCS/sphinx/requirements.txt`); `bandit -r tools/reproduce.py` 0 issues | 0 at gate threshold |
| Sphinx warning-as-error build | 0 warnings | 0 warnings (fresh-directory build; `migration_guide.rst` renders the WP-035 section) | 0 warnings |
| Snyk Code / Snyk Open Source | Snyk Code: S1 ran clean scans; CI authoritative | No dependency manifest changed, so Snyk Open Source is not applicable. CI's Snyk job remains authoritative | 0 at gate threshold |
...
.venv\Scripts\python -m pytest tests/test_reproduce.py --cov=tools.reproduce --cov-report=term-missing --basetemp=.pytest_basetemp-wp035s4 -q   # 23 passed; tools/reproduce.py 100%
.venv\Scripts\python -m pytest tests/ -m "not slow and not gpu" --basetemp=.pytest_basetemp-wp035s4-fast -q   # 719 passed, 9 deselected
.venv\Scripts\python -m pytest tests/ parity/ --basetemp=.pytest_basetemp-wp035s4-full -q   # 1320 passed
.venv\Scripts\python tools/reproduce.py --verify-manifest                     # Verified 172 stored JSON artefacts. Generated 39 files.
.venv\Scripts\python tools/check_deviation_ledger.py DOCS/reports/034-project-state.md   # passed; 117 rows
...
One new finding this cycle: WP035-F1 (D4, private cross-module import, FIXED
in S3). The cumulative table below carries forward every row from
`DOCS/reports/034-project-state.md` §3 unchanged (verified by
`tools/check_deviation_ledger.py`, run against PSR-034) and appends the one
WP035 row.
...
- **DV-022 (ubuntu runner disk exhaustion):** external infrastructure
  condition. Status: OPEN, unchanged — WP-035 changes no CI workflow beyond
  the `repro.yml` guard removal already committed in S1.
- All other DV register items closed at or before PSR-034 remain closed;
  `tools/check_dv_register_gates.py` passes (29 rows / 198 sessions).
- No new risks introduced. WP-035 adds no dependency, no numerics, no
  `unsafe`, no crate change, and no new CI workflow (only activates the
  existing `repro.yml`).

---

## 6. Next work package declaration — WP-036

Quoted directly from the registered successor session brief
(`DOCS/sessions/phase-6/0141-wp036-s1-api-completion-acceptance-suite-and-migration.md`),
per Documentation Standards §7 item 5:

- **Title:** API completion, acceptance suite, and migration.
- **Scope (files/crates/modules):** Per the session brief's Mission —
  finish 175+ symbol mapping, port ~1,670 acceptance tests, API-freeze/
  deprecation machinery, stubs, and symbol-by-symbol Migration Guide data.
- **Plan sections advanced:** §6 (Phase 6 roadmap).
- **Acceptance criteria:** Per the session brief's Contract — every mapped
  symbol resolves and behaves compatibly; full CPU acceptance matrix is
  green; no silent removals or Python numerics.
- **Non-goals:** Per the session brief — final documentation prose or
  release publishing.
- **First session brief:**
  `DOCS/sessions/phase-6/0141-wp036-s1-api-completion-acceptance-suite-and-migration.md`
  (already present in `SESSION_REGISTER.md`/`DOCS/sessions/phase-6/` as
  `PLANNED`, along with its S2–S4 successors 0142–0144).
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-036 S1 begins).
...
(3/6 WPs complete: WP-033, WP-034, WP-035).

**(b) `DOCS/experiments/README.md`:** Verified current — includes
`0137-wp035-s1-handoff.md`.
