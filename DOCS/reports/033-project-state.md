# PRIN Project State Report — Cycle 033

**Date:** 2026-08-26
**Cycle:** 033 (WP-033 "Unified benchmark runner and category migration")
**Completed sessions:** 0129–0132
**Author:** Qwen Code (AI pair), approved by maintainer MichaelMaillet
**Git state:** `main` @ S4 closure (predecessor `5e6148e`; this session's commit follows)

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 6 — Benchmarks, reproduction, docs, and RC1 (**1 of 6**
  phase-6 WPs complete: WP-033).
- **This cycle delivered:**
  - **Unified `benchrunner` CLI** (`benchmarks/benchrunner/`): single entry
    point (`python -m benchmarks.benchrunner`) with `--list`, `--category`,
    `--name`, `--iterations`, `--out` dispatch.
  - **Shared infrastructure** (`benchmarks/_common/`): `BenchmarkConfig`,
    `capture_environment`, `timed_run` (≥10-iteration timing rule, warmup
    exclusion, median/p95), `BenchmarkRegistry` (category/name dispatch),
    `write_result` (output-path confinement to 3 allowed roots).
  - **Nine category packages** with 10 registered benchmarks: `scaling/`
    (2), `chimera/`, `mot/`, `ablations/`, `kernels/` (criterion subprocess
    bridge), `integrators/`, `training/`, `daemon/`, `adversarial/` — all
    delegate to already-parity-dispositioned Rust APIs; no new numerics in
    Python.
  - **Test suite** (`tests/test_benchrunner.py`): 50 tests (49 fast + 1
    slow) covering config validation, timing rule, registry, environment
    capture, result writing, schema compatibility, CLI dispatch, criterion
    bridge parsing, and all remaining categories. 99% coverage on
    `benchmarks/`.
  - **Traceability** (`DOCS/baselines/wp033_benchmark_traceability.md`):
    58-row table mapping every verified legacy PRINet 3.0 benchmark script
    to its category and module (58 verified, not the session brief's quoted
    62 — factual correction, not a plan amendment).
  - **S2 audit** (`DOCS/audits/033-wp033-audit.md`): verdict
    `PASS-WITH-FINDINGS`, one D2 finding (WP033-F1, basetemp confinement).
  - **S3 remediation:** WP033-F1 FIXED (`6eb4e8b`); delta re-audit CLEAN.
  - **S4 documentation:** updated `CHANGELOG.md`,
    `DOCS/sphinx/migration_guide.rst` (new WP-033 section),
    `benchmarks/README.md`, `DOCS/reports/README.md`,
    `DOCS/sessions/phase-6/README.md`,
    `DOCS/sessions/SESSION_REGISTER.md`;
    wrote this report; declared WP-034.
...
- **Audit:** `DOCS/audits/033-wp033-audit.md` — S2 verdict
  `PASS-WITH-FINDINGS` (1 D2 finding). S3 FIXED WP033-F1, delta re-audit
  CLEAN. No unresolved D1/D2 finding exists.
- **Session Register:** 0129 (S1), 0130 (S2), 0131 (S3), 0132 (S4) all
  marked **COMPLETE**; 0133 (WP-034 S1) is the registered successor
...
| Metric | Previous (PSR-032) | Current (PSR-033) | Gate |
|---|---|---|---|
| Rust tests passing | ~1563/~1563 default workspace (excluding `prin-py`), 0 failed, 1 ignored | **~1563/~1563** default workspace (excluding `prin-py`), 0 failed, 1 ignored (unchanged — WP-033 adds no new Rust tests; `kernels/` criterion benches are orchestrated, not re-measured) | 100% where defined |
| Python tests passing | 555 passed, 8 deselected (fast suite); full suite 1155 passed | **624 passed**, 9 deselected (fast suite); full suite **1225 passed**, 0 failed (+69 fast / +70 full from WP-033's 50 `test_benchrunner.py` tests + 19 new differential parity tests from `parity/`) | 100% |
| Coverage (changed code) | `bindings/phase5.rs` 99.62% lines; `bindings/daemon.rs` 100%; Python 100% | `benchmarks/` **99%** lines (607 stmts, 2 miss); all other governed code unchanged | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (266/266) | 100% public (**266/266**) — unchanged; `benchmarks/` is outside governed interrogate scope (consistent with pre-existing policy) | ≥95% overall, 100% public |
...
| Snyk Code / Snyk Open Source | Snyk Code: CI authoritative gate; local scan blocked | Snyk Code: S1 ran clean scans (0 findings on `benchmarks/` and `tests/test_benchrunner.py`); S3 re-confirmed 0 issues on modified test. CI's Snyk job remains authoritative | 0 at gate threshold |

**Verification commands re-run in S4 (2026-08-26, independent of S1/S2/S3 evidence, this host):**
...
.venv\Scripts\python tools/check_dv_register_gates.py                         # 29 rows / 198 sessions
.venv\Scripts\python -m pytest tests/ -m "not slow and not gpu" --basetemp=.pytest_basetemp -q   # 624 passed, 9 deselected
.venv\Scripts\python -m pytest tests/ parity/ --basetemp=.pytest_basetemp-full -q                # 1225 passed
.venv\Scripts\python -m pytest tests/test_benchrunner.py -m "not slow and not gpu" --cov=benchmarks --basetemp=.pytest_basetemp -q  # 49 passed, 1 deselected; benchmarks/ 99%
...
- **DV-019 (flaky gradcheck test):** **CLOSED** — `Hotfix-DV019` session
  completed 2026-08-26 (`5fdfeb0`); cross-thread `AutodiffServer`
  serialization mutex applied to all 42 autodiff-graph-touching tests.
- **DV-021 (bridge overhead `<5%` gap):** CLOSED (amendment #30), unchanged
  this cycle. WP-033 touches no `prin-py` bridge code.
...
- **DV-024 (self-hosted runner offline):** CLOSED — `PRIN-GPU-Runner`
  online; all Windows CI jobs on self-hosted runner.
- **DV-025 (WP-030 traceability drift):** CLOSED at PSR-030, unchanged.
- **DV-029 (`gpu.yml` never fully green):** **CLOSED** — `gpu-cuda` and
  `gpu-wgpu` both `success` for the first time (`dd9979c`, `a5b0794`).

---

## 6. Next work package declaration — WP-034

Quoted directly from the registered successor session brief
(`DOCS/sessions/phase-6/0133-wp034-s1-reporting-figures-tables-and-profiling.md`),
per Documentation Standards §7 item 5:

- **Title:** Reporting, figures, tables, and profiling.
...
- **Non-goals:** Per the session brief.
- **First session brief:**
  `DOCS/sessions/phase-6/0133-wp034-s1-reporting-figures-tables-and-profiling.md`
  (already present in `SESSION_REGISTER.md`/`DOCS/sessions/phase-6/` as
  `PLANNED`, along with its S2–S4 successors 0134–0136).
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-034 S1 begins).
