...
# PRIN Project State Report — Cycle 030

**Date:** 2026-08-25
**Cycle:** 030 (WP-030 "Training hooks and MOT evaluation")
**Completed sessions:** 0117–0120
**Author:** Claude Sonnet 5 (AI pair), approved by maintainer MichaelMaillet
**Git state:** `main` @ S4 closure (predecessor `209802d`; this session's commit follows)

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 5 — Daemon and experiment tooling (**3 of 5**
  phase-5 WPs complete: WP-028, WP-029, WP-030).
- **This cycle delivered:**
  - **Training hooks** (`crates/prin-daemon/src/hooks.rs`): `TrainingHooks`
    — the WP-030 rebuild of PRINet 3.0's `prinet.nn.training_hooks.StateCollector`:
    loss EMA/variance, gradient-norm EMA (from caller-supplied per-parameter
    L2 norms), and step-latency window/p50/p95/throughput, packaged into a
    `SubconsciousState` for `SubconsciousDaemon::submit_state`. Daemon
    integration proven end-to-end by `tests/hooks_daemon_integration.rs`.
  - **MOT evaluation** (`crates/prin-daemon/src/mot.rs`,
    `crates/prin-daemon/src/assignment.rs`): `MotAccumulator`/`MotSummary`
    (MOTA/MOTP/IDF1/identity switches/misses/false positives), `BBox`/
    `iou_distance_matrix`, `Detection`, deterministic `Seed`-driven
    synthetic sequence generators (`generate_linear_sequence`,
    `generate_crowded_sequence`), and a crate-private rectangular
    Hungarian/Kuhn–Munkres assignment solver reproducing
    `motmetrics.lap.add_expensive_edges`'s NaN/Inf handling recipe.
    Validated against real `py-motmetrics` 1.4.0: 10/10 scenarios agree at
    `rtol=1e-9, atol=1e-12`; the generating fixture
    (`tools/wp030_mot_fixture.py`) independently regenerates byte-for-byte
    identical.
  - **S2 audit** (`DOCS/audits/030-wp030-audit.md`): verdict `PASS`, zero
  ...
  - **S4 documentation:** updated `CHANGELOG.md`,
    `DOCS/sphinx/migration_guide.rst` (new WP-030 section),
    `DOCS/reports/README.md`, `DOCS/sessions/SESSION_REGISTER.md`,
    `DOCS/sessions/phase-5/README.md`; discovered and fixed a traceability
    drift (WP030-F1, D4 — see §3); wrote this report; declared WP-031.
- **Plan conformance:** ON TRAJECTORY WITH AMENDMENTS — amendments #1–#30
  remain in force; no new Plan-text amendment was required this cycle (the
  one correction this cycle made, WP030-F1, is a `tools/wp001_ownership.json`
  traceability-map fix, not a Project Plan §6 text change — the Plan's own
  Phase 5 roadmap row never named the affected symbols).
- **Audit:** `DOCS/audits/030-wp030-audit.md` — S2 verdict `PASS`, zero
  findings. S3 no-change closure with CLEAN delta re-audit. No unresolved
  D1/D2 finding exists.
- **Session Register:** 0117 (S1), 0118 (S2), 0119 (S3), 0120 (S4) all
...
| Metric | Previous (PSR-029) | Current (PSR-030) | Gate |
|---|---|---|---|
| Rust tests passing | 1299/1299 default workspace, 0 failed, 1 ignored; `prin-daemon` 156 (106 lib + 4 concurrency + 7 integration + 8 parity + 12 property + 19 doctest) | **1363/1363** default workspace, 0 failed, 1 ignored (unchanged — `prin-train`'s `integration_temporal_clevr_n.rs`); `prin-daemon` **219** (163 lib + 4 concurrency + 2 `hooks_daemon_integration` + 7 `integration_controller_model` + 2 `parity_mot` + 8 `parity_subconscious` + 12 `proptest_properties` + 21 doctest — up from 19; `hooks::TrainingHooks` and `mot::MotAccumulator` each gained one) | 100% where defined |
| Python tests passing | 547 passed, 8 deselected (fast suite); full suite 1147 passed | 547 passed, 8 deselected (fast suite); full suite **1147 passed**, 0 failed (unchanged — WP-030 touches no `python/prin/` source). One isolated flake observed and re-run clean during S4 verification (`TestTemporalSlotAttentionMOT::test_process_frame_gradcheck_with_prev_slots`); see §5 | 100% |
| Coverage (changed code) | `daemon.rs` 97.16%/94.74%/97.28%; other `prin-daemon` files unchanged from PSR-028 | `hooks.rs` **99.75%/100.00%/99.61%**; `mot.rs` **99.72%/100.00%/100.00%**; `assignment.rs` **98.89%/100.00%/98.18%** (all three new this cycle; independently re-measured at S4, identical to S2/S3) | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (262/262) | 100% public (**262/262**) — unchanged (WP-030 touches no `python/prin/` source); `tools/wp030_mot_fixture.py` 100% (9/9); Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` | ≥95% overall, 100% public |
| Parity cases passing / total defined | 510 parity-marked Python tests; 15 `prin-train` golden-value parity tests; 8 `prin-daemon` Rust bit-exact parity tests; 82 differential parity tests | 510 parity-marked Python tests (unchanged); 15 `prin-train` golden-value parity tests (unchanged); 8 `prin-daemon` Rust bit-exact parity tests (WP-028, unchanged) **+ 2 new `prin-daemon` MOT parity tests** (`tests/parity_mot.rs`, 10 scenarios vs. real `py-motmetrics` 1.4.0, `rtol=1e-9, atol=1e-12`); 82 differential parity tests (unchanged) | 100% at tolerance |
...
| Sphinx warning-as-error build | 0 warnings | 0 warnings (fresh-directory build, re-run twice at S4 — once before, once after the migration-guide correction) | 0 warnings |
| Snyk Code / Snyk Open Source | Snyk Code: CI authoritative gate; local scan blocked. Snyk OS: N/A for Cargo | Snyk Code: local re-scan **BLOCKED** (standing condition since WP-001, R23; no interactive Snyk CLI/MCP session available in this non-interactive execution environment — reported as blocked, not claimed passed). CI's Snyk job remains authoritative. Ecosystem-native `cargo audit`/`pip-audit` both clean (above) | 0 at gate threshold |
| Benchmark regression gates | none defined for `prin-daemon`; none tripped | `benches/training_hooks.rs` (criterion, per-call cost of `on_step_end_with_elapsed`/`on_epoch_end`); no regression gate defined — pilot evidence only, consistent with `benches/control_buffer.rs`; none tripped | none tripped |
...
cargo test --workspace -- --test-threads=1                                    # 1363 passed, 0 failed, 1 ignored
cargo test --workspace --features strict-checks -- --test-threads=1           # 1364 passed (prin-daemon +1 strict-only test), 0 failed
cargo llvm-cov -p prin-daemon --features strict-checks                        # hooks.rs 99.75%, mot.rs 99.72%, assignment.rs 98.89% (unchanged from S2/S3)
...
.venv\Scripts\python -m pytest tests/ -m "not slow and not gpu" --basetemp=.pytest_basetemp   # 547 passed, 8 deselected (one isolated flake on first attempt, clean on immediate re-run — see §5)
.venv\Scripts\python -m pytest tests/ parity/ --basetemp=.pytest_basetemp-full                # 1147 passed
Remove-Item -Recurse -Force DOCS/sphinx/_build; python -m sphinx.cmd.build -W --keep-going -b html DOCS/sphinx DOCS/sphinx/_build/html   # build succeeded, 0 warnings (re-run twice)
.venv\Scripts\python tools/wp001_baseline.py check                            # WP-001 baseline validation passed (after the DV-025 ownership.json fix)
.venv\Scripts\python tools/wp030_mot_fixture.py crates/prin-daemon/tests/data/mot_reference_cases_regenerated.json  # Wrote 10 scenarios; byte-identical to the committed fixture
...
tools/check_deviation_ledger.py DOCS/reports/029-project-state.md DOCS/reports/030-project-state.md   # run after this report was written; see §3
```
...
DV-024) remains the authoritative gate. Session 0119 (S3) did have Snyk
MCP access and recorded clean scans of `crates/prin-daemon` and
`python/prin`; nothing in this WP's source has changed since.

---

## 3. Deviation ledger (cumulative)

One new finding was raised and closed this cycle. **WP030-F1** (D4) —
discovered during the S4 documentation-accuracy sweep (Documentation
Standards §7 item 8), not during S2 (S2 recorded zero findings; this is a
post-audit documentation-currency correction, not a reopening of the S2
verdict). The cumulative table below carries forward every row from
`DOCS/reports/029-project-state.md` §3 unchanged (verified by
`tools/check_deviation_ledger.py`, run in two-report mode against this
report) and appends the one new row.
...
#1–#30 remain in force. WP030-F1's fix is a `tools/wp001_ownership.json`
traceability-map correction, not a Plan-text change — the Plan's own Phase 5
roadmap row never named `export_to_onnx`/`quantize_onnx`/`retrain_controller`,
so no roadmap text was ever wrong; only the WP-028-era ownership override was.

---

## 5. Risks and deferred items

- **DV-005 (CUDA Burn backend):** unchanged — deferred out of Phase 5,
  re-gated at Phase 6 (WP-036) per the R31 disposition (WP-028 S1, 2026-08-21).
  Status: OPEN. WP-030 introduces no workload needing a CUDA Burn backend.
- **DV-006 (DirectML/VitisAI ONNX validation):** unchanged — DirectML cannot
  execute the controller graph, VitisAI is not registered on this host.
  Status: OPEN — re-gated to WP-032 S1. WP-030 touches no ONNX export/graph
  code, so this gap is unaffected by this cycle.
...
- **DV-019 (gradient-flow test flake):** re-checked. Did not recur in Rust
  `cargo test --workspace` this cycle. **Recurred in a new, Python-side
  location** during S4 verification —
  `tests/test_train_bridge_slot_attention.py::TestTemporalSlotAttentionMOT::test_process_frame_gradcheck_with_prev_slots`
  failed once, then passed in isolation and on an immediate full-suite
  re-run (547 passed, 8 deselected, matching this cycle's own baseline
  exactly). This S4 session touches zero Rust/Python source, so it cannot be
  a regression introduced here. Recorded in the DV register as an
  unconfirmed possible same-class instance (floating-point summation-order
  sensitivity under parallel test-thread contention in the shared
  Burn/`NdArray` backend), not folded into DV-019's existing root-cause
  narrative pending confirmation. Status: OPEN — non-blocking, unchanged
  disposition (dedicated future hotfix/correction session, not a drive-by
  fix in an unrelated WP's cycle).
...
- **DV-025 (new this cycle):** `SubconsciousController.export_to_onnx`/
  `.quantize_onnx`/`retrain_controller` traceability drift. **CLOSED same
  session** — see §3 (WP030-F1) and `DOCS/reports/DEFERRED_VALIDATION_REGISTER.md`
  DV-025 for full evidence. Re-targeted to **WP-036**.

---

## 6. Next work package declaration — WP-031

Quoted directly from the registered successor session brief
(`DOCS/sessions/phase-5/0121-wp031-s1-temporal-experiments-statistics-and-adversarial-tooling.md`),
per Documentation Standards §7 item 5:

- **Title:** Temporal experiments, statistics, and adversarial tooling.
...
- **Acceptance criteria:** Per the session brief's Contract: "Statistical
  routines match trusted references; matched-budget controls are enforced;
  attack bounds and deterministic multi-seed behavior are tested."
- **Non-goals:** Per the session brief's Contract: "Running the final
  scientific campaign."
- **First session brief:**
  `DOCS/sessions/phase-5/0121-wp031-s1-temporal-experiments-statistics-and-adversarial-tooling.md`
  (already present in `SESSION_REGISTER.md`/`DOCS/sessions/phase-5/README.md`
  as `PLANNED`, along with its S2–S4 successors 0122–0124).
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-031 S1 begins — this session was executed at the
  ...
  project's own precedent (WP-028 S1, WP-029 S1, WP-030 S1 handoffs) treats
  as an in-session approval when no separate written sign-off exists yet).
...
(WP-030 is the third of five Phase 5 WPs), so item 9's explicit verification
...
This cycle's own documentation-accuracy gap (WP030-F1/DV-025) was found by
item 8's ordinary per-cycle sweep (comparing the Migration Guide's
WP-029-section deferral bullets against the live-generated traceability
output), not by item 9's phase-closing sweep — recorded here for
completeness, not because item 9 applied.
