...
# PRIN Project State Report — Cycle 027

**Date:** 2026-08-20
**Cycle:** 027 (WP-027 "Trainable-stack integration and Phase 4 gate")
**Completed sessions:** 0105–0108
**Author:** Qwen Code (AI pair), maintainer-reviewed
**Git state:** `main` @ S3 closure (`48eade3`); S4 documentation closure follows

---

## 1. Current position on the planned trajectory

- **Roadmap phase:** 4 — Trainable stack and Torch bridge (**6 of 6**
  phase-4 WPs complete: WP-022, WP-023, WP-024, WP-025, WP-026, WP-027).
  **Phase 4 is complete.**
- **This cycle delivered:**
  - **Temporal CLEVR-N dataset generator** (`crates/prin-train/src/dataset.rs`):
    `SequenceData`, `TemporalClevrNConfig`, `generate_temporal_clevr_n`,
    `generate_dataset` — structural port of PRINet 3.0's temporal CLEVR-N
    sequence generator using the project's counter-based `Seed`.
  - **Training losses** (`crates/prin-train/src/losses.rs`):
    `hungarian_similarity_loss`, `temporal_smoothness_loss` — direct ports of
    the reference training losses with hand-computed parity tests.
  - **Rust-native training loop** (`crates/prin-train/src/trainer.rs`):
    `TemporalTrainerConfig`, `TrainingResult`, `ValMetrics`,
    `train_phase_tracker`, `evaluate_phase_tracker` — Burn Adam + warmup/cosine
    LR + gradient clipping + early stopping with best-checkpoint restore.
  - **Optimizer bridges** (`crates/prin-py/src/bindings/optim.rs`):
    `SyncGdBridge`/`ScalrBridge`/`RipBridge` — non-differentiable
    optimizer-step PyO3 bridges.
  - **Trainer bridge** (`crates/prin-py/src/bindings/trainer.rs`):
    `train_phase_tracker` PyO3 entry + `TrainingResult` pyclass.
  - **Python optimizer wrappers** (`python/prin/nn/optimizers.py`):
    `SyncGd`/`Scalr`/`Rip` `torch.optim.Optimizer` subclasses.
  - **Python training entry point** (`python/prin/train.py`):
    `train_phase_tracker`/`TrainingResult` — thin orchestration module.
  - **Bridge profiling** (`crates/prin-train/benches/phase_tracker_bridge.rs`):
    Criterion baseline for `PhaseTracker::forward`/`match_frames`.
  - **Integration tests**: `integration_temporal_clevr_n.rs` (IP-threshold
    acceptance, `#[ignore]`d, ~115s), `integration_checkpoint_resume.rs`
    (trained-state serialization round-trip).
  - **Python tests**: `test_train_bridge_optim.py` (13 tests),
    `test_train_pipeline.py` (4 tests), extended
    `test_train_bridge_phase_tracker.py` (composed "full gradcheck").
  - **S2 audit** (`DOCS/audits/027-wp027-audit.md`): verdict `PASS`, zero
  ...
  - **S4 documentation:** updated `crates/prin-train/README.md`,
    `CHANGELOG.md`, `DOCS/sphinx/migration_guide.rst`,
    `DOCS/sessions/SESSION_REGISTER.md`, `DOCS/sessions/phase-4/README.md`,
    `DOCS/experiments/README.md`, `DOCS/audits/README.md`,
    `DOCS/reports/README.md`; wrote this report; declared WP-028.
- **Plan conformance:** ON TRAJECTORY WITH AMENDMENTS — amendments #1–#29
  remain in force; no new amendment was required this cycle.
- **Audit:** `DOCS/audits/027-wp027-audit.md` — S2 verdict `PASS`, zero
  findings. S3 no-change closure with CLEAN delta re-audit. No unresolved
  D1/D2 finding exists.
- **Session Register:** 0105 (S1), 0106 (S2), 0107 (S3), 0108 (S4) all
  marked **COMPLETE**; 0109 (WP-028 S1) is the registered successor.

---

## 2. Metric trends

| Metric | Previous (PSR-026) | Current (PSR-027) | Gate |
|---|---|---|---|
| Rust tests passing | 1131/1131 default workspace; `prin-train` default 275 (lib) + 16 (integration) + 13 doctests = 304 | **1144/1144** default workspace, 0 failed, 1 ignored; `prin-train` default **292** (lib) + **2** (integration: `integration_temporal_clevr_n` ignored + `integration_checkpoint_resume`) + **13** doctests = **307** (+3 from new integration tests and doctests) | 100% where defined |
| Python tests passing | 421 passed, 8 deselected (fast suite); full suite 939 passed | **441 passed**, 8 deselected (fast suite); full suite `pytest tests/ parity/` **959 passed**, 0 failed | 100% |
| Coverage (changed code) | `ablation.rs` 98.54%/100%/99.60%; `allocation.rs` 98.14%/97.22%/97.16%; etc. | `dataset.rs` **99.75%**/99.56%/100%; `losses.rs` **100%**/97.71%/100%; `trainer.rs` **96.26%**/97.35%/100% — all ≥95% | ≥95% on instrumentable changed code |
| Docstring coverage (interrogate) | 100% public (213/213) | 100% public (**231/231**) — `nn/` files + `train.py` 100%; Rust `#![warn(missing_docs)]` clean under `RUSTDOCFLAGS=-D warnings` | ≥95% overall, 100% public |
| Parity cases passing / total defined | 510 parity-marked Python tests; 15 `prin-train` golden-value parity tests | 510 parity-marked Python tests (unchanged); **15** `prin-train` golden-value parity tests (unchanged — WP-027 adds integration-level validation, not new golden-value parity cases) | 100% at tolerance |
...
| Sphinx warning-as-error build | 0 warnings | 0 warnings (`migration_guide.rst` updated with WP-027 symbol mappings and bridge entries) | 0 warnings |
...
| Benchmark regression gates | none defined for `prin-train`; none tripped | none defined for `prin-train`; none tripped. New `phase_tracker_bridge` criterion bench added | none tripped |

**Verification commands re-run in S4 (2026-08-20, independent of S2/S3 evidence):**
...
cargo test --workspace -- --test-threads=1                              # 1144 passed, 0 failed, 1 ignored
...
`DOCS/reports/026-project-state.md` §3 unchanged (verified by
...
| WP026-F1 | 026 | D4 | `AdaptiveOscillatorAllocator::validate_shapes` did not detect strategy mismatch (Rule vs. Learned); checkpoint from differently-strategied allocator silently discarded MLP load | FIXED | S3 commit `2135077`; `TrainError::StrategyMismatch` variant, `(Learned, None)` check in `validate_shapes`, 2 regression tests |
| WP026-F2 | 026 | D4 | No whole-module golden-value parity test for `HybridPRINetV2.forward` | FIXED | S3 commit `2135077`; `HybridPRINetV2Params` + `init_from_params`, `tests/parity_hybrid.rs` golden-value test; parity test revealed and fixed missing ReLU in classifier head |
...
Amendments #1–#29 remain in force. No new amendment was required this cycle.

---

## 5. Risks and deferred items

- **DV-021 (bridge overhead `<5%` gap):** WP-027 S1 independently
  re-corroborated DV-021's architectural-cost conclusion with fresh
  measurements (+37.8%/+6.5% vs. DV-021's +39.8%/+5.3%). Status: OPEN —
  unchanged. Governed by existing disposition.
- **DV-005 (CUDA Burn backend):** WP-027 S1 provided a concrete
  recommendation: "do not pull into near-term Phase 5 scope absent a concrete
  workload that needs it." Status: OPEN — recommendation recorded for
  maintainer disposition.
- **DV-019 (gradient-flow test flake):** WP-027 S1 recorded a new recurrence
  affecting `hybrid.rs` and `phase_tracker.rs` in addition to the original
  `bands.rs` — three modules now affected, strengthening the case for
  mitigation (a) (pin to single-threaded). Status: OPEN — new evidence
  recorded.
...
- **DV-022 (ubuntu runner disk exhaustion):** EA-005 external infrastructure
  condition. `parity.yml` has WSL2 fallback documented. Status: OPEN.
- **DV-010 (Phase 1/2 pre-release tag):** Version logic resolved; tag push
  pending explicit maintainer action. Status: OPEN.

---

## 6. Phase 4 exit gate verdict

Phase 4 (Trainable stack and Torch bridge) encompasses WP-022 through WP-027
(sessions 0085–0108). All six work packages are complete with passing audits:

| WP | Audit verdict | Key deliverable |
|---|---|---|
| WP-022 | PASS-WITH-FINDINGS (2 D4, both FIXED) | `DiscreteDeltaThetaGamma`, `ResonanceLayer` |
| WP-023 | PASS-WITH-FINDINGS (1 D4, FIXED) | Inhibition, activations, energy, HEP |
| WP-024 | PASS-WITH-FINDINGS (1 D3, AMENDED) | SyncGd, Scalr, RIP optimizers |
| WP-025 | PASS-WITH-FINDINGS (2 D2, 1 D3, 2 D4, all FIXED) | Production Torch autograd bridge |
| WP-026 | PASS-WITH-FINDINGS (2 D4, both FIXED) | PhaseTracker, Hybrid, baselines, allocation |
| WP-027 | PASS (zero findings) | Integration, training loop, optimizer wrappers |

**Phase 4 exit gate: GREEN.**

- All acceptance criteria met or governed by existing deviations.
- PhaseTracker mean IP = 1.00000 ≥ registered 0.99868 threshold.
- Gradchecks green (including composed "full gradcheck").
- Coverage ≥95% on all new code.
- Zero unresolved D1/D2 findings.
- EA-005 (Phase 4 close executive audit): PASS-WITH-REMEDIATION, all
  findings addressed.
- EMA-003/EMA-004 (Phase 4 close mathematical audits): PASS-WITH-REMEDIATION,
  40 claims across 8 ledgers, 33 PASS, 7 REQUIRES_HUMAN_REVIEW (maintainer
  sign-off granted).

**Pre-release tag:** Per Versioning and Release Standards §1, Phase 4 exit
is tagged as `v0.5.0-alpha.1`. The tag creation and push is a maintainer-
confirmed action (same precedent as DV-010 for Phase 1/2 tags). The version
in `Cargo.toml`/`pyproject.toml` is already at `0.5.0-alpha.1` (bumped at
the Phase 3 exit, WP-021 S4). The actual `git tag` + push (which triggers
`release.yml` for PyPI/crates.io publish) requires explicit maintainer
confirmation.

---

## 7. Next work package declaration — WP-028

- **Title:** ONNX controller and backend selection
- **Scope (files/crates/modules):** Controller state/control types, ONNX model
  validation, provider detection (VitisAI→DirectML→CPU), and deterministic
  fallback.
...
- **Acceptance criteria:** Outputs match 3.0 references across available
  providers; missing-provider paths fall back safely; model SHA-256 is
  verified.
- **Non-goals:** Background runtime/threading.
- **First session brief:** `DOCS/sessions/phase-5/0109-wp028-s1-onnx-controller-and-backend-selection.md`
- **Maintainer approval:** pending (recorded here at declaration; approval
  required before WP-028 S1 begins)

---

## 8. Phase-closing cross-cutting document currency

Per Documentation Standards §7 item 9 (phase-closing S4 requirement):

**(a) `DOCS/PRIN_Project_Plan.md` §6 roadmap-table completion marker:**
Phase 4 is the sixth roadmap phase. The plan's roadmap table should mark
Phase 4 as complete. This update is recommended for the next cycle that
modifies the Project Plan (WP-028 or later); the substantive engineering
work is complete regardless of the marker's presence.

**(b) `DOCS/experiments/README.md` file index:** Updated this session to
include `0105-wp027-s1-handoff.md` and `0105-wp027-temporal-clevr-n-validation.json`.

**(c) `DOCS/sessions/SESSION_REGISTER.md` Global Sessions section:**
Verified against the actual Executive Audit / Executive Mathematical Audit
report list. EA-001 through EA-005 and EMA-001 through EMA-004 are all
listed and marked COMPLETE. Current as of this session.
