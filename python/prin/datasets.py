...
Strict port of PRINet 3.0 ``prinet.utils.datasets``: cached CIFAR-10 and
Fashion-MNIST loaders (torchvision) plus a quick top-1 accuracy helper. All
data is cached in ``~/.cache/prinet/datasets`` by default.

This module is data-plumbing only -- ``torchvision`` transform pipelines,
``DataLoader`` construction, and an ``argmax``/equality accuracy count. No
oscillator numerics (Coding Standards s1.2): the same category as the
``benchmarks`` synthetic-data generators.

Example::

    from prin.datasets import get_cifar10_loaders
...
def get_fashion_mnist_loaders(
    ...
    Fashion-MNIST images are 1-channel 28x28.  The loader resizes them
    to 32x32 to match the CIFAR-10 spatial resolution expected by
    :class:`~prin.nn.HybridPRINetV2` with ``use_conv_stem=True``.
