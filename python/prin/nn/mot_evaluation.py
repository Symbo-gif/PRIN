"""MOT17 evaluation pipeline for PRIN's ``PhaseTracker``.

Strict port of PRINet 3.0 ``prinet.nn.mot_evaluation``: synthetic MOT
sequence generators (for quick evaluation without downloading the full MOT17
dataset), a MOT17 annotation loader, the non-oscillatory ``AttentionTracker``
comparison baseline, and the ``evaluate_tracking`` / ``run_subconscious_ab_test``
harness.

Division of labour (Coding Standards s1.2): every MOT *metric*
(MOTA / MOTP / IDF1 / identity switches / misses / false positives) is
computed by the Rust ``prin_daemon::mot`` core, reached here through
:class:`prin.eval.MotAccumulator` -- the "``python/prin/eval`` orchestration"
facade the WP-030 migration guide flagged as *not yet delivered*. This module
supplies only the tracker-wiring loop and the synthetic-data / baseline
tooling, exactly the same category as the ``benchmarks`` generators and the
``benchmarks.clevr_n`` LSTM/Transformer comparison baselines. ``AttentionTracker``
is a deliberate non-oscillatory baseline (used at random init, never trained
in the acceptance suite), mirroring PRINet 3.0.
"""
...
from prin.eval import MotAccumulator
...
def generate_temporal_reasoning_sequence(
    ...
    detection_dim: int = 4,
    noise_std: float = 0.02,
    occlusion_frames: list[tuple[int, int, int]] | None = None,
    distractor_injection_frames: list[tuple[int, int]] | None = None,
    seed: int = 42,
) -> list[list[Detection]]:
...
def evaluate_tracking(
    ...
    Runs the tracker frame-by-frame, builds a prediction track table, and
    computes MOTA, IDF1, ID switches, etc. through the Rust MOT metrics core
    (:class:`prin.eval.MotAccumulator`).
    ...
    prev_dets_tensor: Tensor | None = None
    ...
    for t, frame_dets in enumerate(sequence):
        ...
        if prev_dets_tensor is None or prev_dets_tensor.shape[0] == 0:
            ...
            hyp_id_list = [
                tid
                for tid, gid in zip(frame_track_ids, gt_ids, strict=False)
                if gid >= 0
            ]
            ...
            if gt_id_list and hyp_id_list:
                ...
                acc.update(t, gt_id_list, hyp_id_list, dists)
            else:
                acc.update(t, [], [], [])
        ...
        # Compute distance matrix for the MOT core
        gt_id_list = [g for g in gt_ids if g >= 0]
        hyp_id_list = [
            tid for tid, gid in zip(frame_track_ids, gt_ids, strict=False) if gid >= 0
        ]
        ...
        if gt_id_list and hyp_id_list:
            ...
            acc.update(
                ...
                gt_id_list,
                hyp_id_list,
                [[] for _ in gt_id_list] if gt_id_list else [],
            )
    ...
    mota_val = float(summary.mota)
    motp_val = float(summary.motp)
    idf1_val = float(summary.idf1)
    idsw_val = int(summary.num_switches)
    fp_val = int(summary.num_false_positives)
    fn_val = int(summary.num_misses)
...
class AttentionTracker(nn.Module):
    ...
    def __init__(
        ...
        """Build the two-layer detection embedding MLP."""
        super().__init__()
        ...
        for frame_dets in sequence:
            ...
            for det in frame_dets:
                ...
                if det.features is not None:
                    ...
                    noisy_feats = [
                        f + n for f, n in zip(det.features, noise, strict=False)
                    ]
                else:
