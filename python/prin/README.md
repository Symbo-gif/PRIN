...
| `dynamics.py` | Re-export module for oscillator state, models, integrators (incl. exponential/multi-rate, WP-012), coupling, PAC (WP-009), and continuous hierarchical band networks + complex-phasor temporal propagation (WP-013) — 27 symbols from `prin._prin_core` | 1, 2 |
...
| `nn/` | torch wrappers, `autograd.Function` bridges, SlotAttention baselines, optimizer wrappers (`SyncGd`/`Scalr`/`Rip`, WP-027) | 4 |
| `train.py` | Thin Python entry point for the Rust-native trainer (`train_phase_tracker`/`TrainingResult`, WP-027) | 4 |
