...
Every figure here is derived output, not a source file. From the repository
root:
...
This reads the stored JSON artefacts, checks all 172 of them against the
governed SHA-256 manifest in `../artefact_manifest.json`, and renders the 14
figures deterministically (fixed 2000-01-01 PDF date, `Agg` backend, no
training, no GPU, no random sampling). The generated `*.pdf` and `*.png` are
