...
class OscilloBench:
    ...
    def run(
        ...
        for task_name in tasks:
            ...
            try:
                task_result = self._task_registry[task_name](model_factory, model_name)
    ...
    def _run_xor_n(self, model_factory: Any, model_name: str) -> dict[str, Any]:
    ...
    def _run_random_dichotomies(
        ...
        per_N: list[dict[str, Any]] = []
        for N in N_values:
            X_train, y_train = make_random_dichotomies(N, n_dims=32, seed=self.seed)
            X_test, y_test = make_random_dichotomies(64, n_dims=32, seed=self.seed + 1)
    ...
    def _run_mnist(self, model_factory: Any, model_name: str) -> dict[str, Any]:
    ...
    def _run_fashion_mnist(self, model_factory: Any, model_name: str) -> dict[str, Any]:
        """Run synthetic Fashion-MNIST benchmark."""
        torch.manual_seed(self.seed)
        X_train, y_train = make_synthetic_fashion_mnist(n_train=2000, seed=self.seed)
        X_test, y_test = make_synthetic_fashion_mnist(n_train=500, seed=self.seed + 1)
        model = model_factory(n_dims=784, n_classes=10)
        metrics = train_and_evaluate(
            model,
            X_train,
    ...
    def _run_clevr_n(self, model_factory: Any, model_name: str) -> dict[str, Any]:
        ...
        for results in results_list:
            ...
            for task_name, task_data in results.get("tasks", {}).items():
                if task_data.get("status") == "ERROR":
                    lines.append(f"| {model_name} | {task_name} | status | ERROR |")
                    continue
                if "results" in task_data:
                    for entry in task_data["results"]:
                        key = (
                            entry.get("n")
                            or entry.get("N")
                            or entry.get("n_items", "?")
                        )
                        acc = entry.get("test_acc", 0.0)
                        lines.append(
                            f"| {model_name} | {task_name} (n={key}) "
                            f"| test_acc | {acc:.3f} |"
                        )
                else:
...
def make_xor_n(
    n: int, *, n_samples: int = 256, seed: int = SEED
) -> tuple[Tensor, Tensor]:
...
class LSTMBaseline(nn.Module):
    """LSTM baseline treating 784-d input as a 28x28 sequence.
    ...
    def __init__(
        ...
        self.lstm = nn.LSTM(self.feature_dim, hidden_dim, n_layers, batch_first=True)
...
def benchmark_xor_n() -> dict[str, Any]:
    ...
    for n_bits in [2, 3, 4, 5, 6]:
        ...
        print(
            f"    XOR-{n_bits}: PRINet acc={prinet_metrics['test_acc']:.3f}"
...
def benchmark_random_dichotomies() -> dict[str, Any]:
    ...
    for n_points in [32, 64, 128]:
        ...
        print(
            f"    N={n_points}: PRINet acc={prinet_metrics['test_acc']:.3f}"
            f"  MLP acc={mlp_metrics['test_acc']:.3f}"
        )
