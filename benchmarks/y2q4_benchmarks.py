        ...
        if i % 2 == 0:
            if n_objects <= max_unique:
                ...
                extras = torch.randint(
                    0, max_unique, (n_objects - max_unique,), generator=rng
                ).tolist()
                colors = base + extras
...
def run_j3_regression_suite(
    ...
    print(
        f"  Accuracy: {final_acc:.1f}% (threshold: 40%) — {'PASS' if goldilocks_pass else 'FAIL'}"
    )
    ...
    print(
        f"  Latency: {elapsed:.3f} ms/batch (threshold: 50ms) — {'PASS' if throughput_pass else 'FAIL'}"
    )
    ...
    for model_name, factory in model_factories.items():
        ...
        for N in n_range:
            ...
            try:
                ...
                print(
                    f"  N={N:3d}: {acc:5.1f}%{'  <-- below threshold' if acc < acc_threshold else ''}"
                )
