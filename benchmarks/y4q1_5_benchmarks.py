...
def run_timed_session(
    ...
    while True:
        ...
        for m in pt_result.get("identity_matches", []):
            pt_matches_interval.append(
                m if isinstance(m, torch.Tensor) else torch.tensor(m, dtype=torch.long)
            )
        for m in sa_result.get("identity_matches", []):
            sa_matches_interval.append(
                m if isinstance(m, torch.Tensor) else torch.tensor(m, dtype=torch.long)
            )
        pt_sims_interval.extend(pt_result.get("per_frame_similarity", []))
        sa_sims_interval.extend(sa_result.get("per_frame_similarity", []))
        pt_rho_interval.extend(pt_result.get("per_frame_phase_correlation", []))
        ...
        if interval_frame_count >= SAMPLE_INTERVAL_FRAMES:
            ...
            entry: dict[str, Any] = {
                ...
                "fps": round(interval_frame_count / max(interval_wall_sec, 1e-9), 2),
            ...
            if len(pt_phases_interval) >= 2:
                ...
                if mean_phase.shape[1] >= 2:
                    plv = phase_locking_value(mean_phase[:, 0], mean_phase[:, 1])
            ...
            if pt_matches_interval:
                ...
                pt_possible = sum(m.shape[0] for m in pt_matches_interval)
                entry["pt_ip"] = round(pt_matched / max(pt_possible, 1), 6)

            if sa_matches_interval:
                sa_matched = sum(
                    int((m >= 0).sum().item()) for m in sa_matches_interval
                )
                sa_possible = sum(m.shape[0] for m in sa_matches_interval)
                entry["sa_ip"] = round(sa_matched / max(sa_possible, 1), 6)
    ...
    if interval_data:
        ...
        agg["fps_degradation_pct"] = (
            round(
                ((fps_list[-1] - fps_list[0]) / max(abs(fps_list[0]), 1e-9)) * 100,
                2,
            )
            if len(fps_list) >= 2
            else 0.0
        ...
        agg["pt_mean_slip_fraction"] = round(float(np.mean(pt_slip_list)), 6)
        agg["pt_max_slip_fraction"] = round(float(np.max(pt_slip_list)), 6)
...
def _run_duration_benchmark(
    ...
    from prin.y4q1_tools import bootstrap_ci
    ...
    ci_pt_ip = (
        bootstrap_ci(ip_pt_all)
        if len(ip_pt_all) >= 2
        else {"mean": ip_pt_all[0] if ip_pt_all else 0, "ci_lower": 0, "ci_upper": 0}
    )
    ci_sa_ip = (
        bootstrap_ci(ip_sa_all)
        if len(ip_sa_all) >= 2
        else {"mean": ip_sa_all[0] if ip_sa_all else 0, "ci_lower": 0, "ci_upper": 0}
    )
    ci_fps = (
        bootstrap_ci(fps_all)
        if len(fps_all) >= 2
        else {"mean": fps_all[0] if fps_all else 0, "ci_lower": 0, "ci_upper": 0}
    )
    ci_r = (
        bootstrap_ci(r_all)
        if len(r_all) >= 2
        else {"mean": r_all[0] if r_all else 0, "ci_lower": 0, "ci_upper": 0}
    )
    ...
    summary = {
        ...
        "fps_ci": ci_fps,
        "order_param_ci": ci_r,
        "mean_slip_fraction": round(sum(slip_all) / max(len(slip_all), 1), 6),
    }
...
def benchmark_10min() -> dict:
    ...
    _save(
        "10min",
        {
            ...
            "description": ("10-minute continuous tracking session. 3 seeds."),
            "results": result,
        },
    )
    return result
...
def benchmark_30min() -> dict:
    ...
    _save(
        "30min",
        {
            ...
            "description": ("30-minute continuous tracking session. 3 seeds."),
            "results": result,
        },
    )
    return result
...
def benchmark_60min() -> dict:
    ...
    _save(
        "60min",
        {
            ...
            "description": ("60-minute continuous tracking session. 3 seeds."),
            "results": result,
        },
    )
    return result
...
def benchmark_cross_duration_comparison(
    ...
    """
    from prin.y4q1_tools import session_length_statistical_comparison
