...
PT_KWARGS = dict(
    n_delta=4, n_theta=8, n_gamma=16, n_discrete_steps=5, match_threshold=0.1
)
SA_KWARGS = dict(num_slots=6, slot_dim=64, num_iterations=3, match_threshold=0.1)
...
def _build_pt_large(seed: int = 42) -> nn.Module:
    ...
    from prin.y4q1_tools import PhaseTrackerLarge
...
def _build_pt_static(seed: int = 42) -> nn.Module:
    ...
    from prin.nn.ablation_variants import create_ablation_tracker

    torch.manual_seed(seed)
    return create_ablation_tracker("pt_static", detection_dim=DET_DIM, **PT_KWARGS)


def _gen(
    n: int, n_frames: int = N_FRAMES, n_objects: int = N_OBJECTS, **kw: Any
) -> list:
    """Generate temporal CLEVR-N sequences."""
    from prin.temporal_training import generate_dataset

    return generate_dataset(
        n, n_objects=n_objects, n_frames=n_frames, det_dim=DET_DIM, **kw
    )
...
def _eval_ip(model: nn.Module, dataset: list, device: str = DEVICE) -> list[float]:
...
def _welch_t(a: list[float], b: list[float]) -> dict[str, float]:
    ...
    if se < 1e-15:
        ...
        df_den = (var_a / n_a) ** 2 / max(n_a - 1, 1) + (var_b / n_b) ** 2 / max(
            n_b - 1, 1
        )
    ...
    cohens_d = (
        float(
            (mean_a - mean_b)
            / np.sqrt(((n_a - 1) * var_a + (n_b - 1) * var_b) / max(n_a + n_b - 2, 1))
        )
        if (var_a + var_b) > 0
        else 0.0
    )
...
def bench_g1_1_7seed_comparison() -> dict:
    ...
    result = {
        ...
        "per_seed": per_seed,
        "conclusion": (
            "significant_pt_advantage"
            if stats["p_value"] < 0.05 and stats["cohens_d"] > 0
            else "significant_sa_advantage"
            if stats["p_value"] < 0.05 and stats["cohens_d"] < 0
    ...
    _p(
        f"  Welch t={stats['t_stat']:.3f}, p={stats['p_value']:.4f}, d={stats['cohens_d']:.3f}"
    )
    return result
...
def bench_g1_2_7seed_noise() -> dict:
    ...
    for sigma in sigmas:
        ...
        for seed in SEEDS_3:
            ...
            sa.load_state_dict(sa_state)
            sa.eval().to(DEVICE)
            test_data = _gen(TEST_SEQS, base_seed=seed + 80000, noise_sigma=sigma)
        ...
        _p(
            f"    PT: {np.mean(pt_ips):.4f}, SA: {np.mean(sa_ips):.4f}, "
...
def bench_g2_1_embedding_analysis() -> dict:
    ...
    results["diagnosis"] = (
        "embedding_collapse"
        if large_disc < small_disc * 0.5
        else "reduced_discriminability"
        if large_disc < small_disc
    ...
    for cfg in configs:
        ...
        for seed in SEEDS_3:
            ...
            for occ_rate in occlusion_rates:
                test_data = _gen(
                    TEST_SEQS, base_seed=seed + 90000, occlusion_rate=occ_rate
                )
        ...
        ablation_results.append(
            {
                "config": cfg,
                "per_occlusion": {
                    str(occ): _bootstrap_ci(seed_ips[occ]) for occ in occlusion_rates
                },
            }
        )
...
def bench_g3_1_object_scaling() -> dict:
    ...
    for n_obj in object_counts:
        ...
        for seed in SEEDS_3:
            ...
            test_data = _gen(TEST_SEQS, base_seed=seed + 60000, n_objects=n_obj)
            pt_ip = sum(_eval_ip(pt, test_data)) / TEST_SEQS
            sa_ip = sum(_eval_ip(sa, test_data)) / TEST_SEQS
            pt_ips.append(pt_ip)
            sa_ips.append(sa_ip)
    ...
    result = {
        ...
        "conclusion": (
            "pt_scales_better"
            if (pt_breaking is None and sa_breaking is not None)
            else "sa_scales_better"
            if (sa_breaking is None and pt_breaking is not None)
            else "both_robust"
            if (pt_breaking is None and sa_breaking is None)
            else "both_degrade"
        ),
    }
...
def bench_g3_2_sequence_scaling() -> dict:
    ...
    for n_frames in frame_counts:
        ...
        for seed in SEEDS_3:
            ...
            sa.eval().to(DEVICE)
            test_data = _gen(
                max(TEST_SEQS // 2, 4), n_frames=n_frames, base_seed=seed + 65000
            )
...
def bench_g4_1_chimera_in_tracking() -> dict:
    ...
    with torch.no_grad():
        for seq_i, seq in enumerate(test_data):
            ...
            chi_threshold = 0.5
            chi = sum(1 for r in per_osc_r if r < chi_threshold) / max(
                len(per_osc_r), 1
            )
            ...
            per_seq_metrics.append(
                {
                    ...
                    "inter_object_coherence": float(np.mean(inter_r))
                    if inter_r
                    else 0.0,
                    "coherence_ratio": (
...
def bench_g5_1_fine_occlusion() -> dict:
    ...
    occlusion_rates = [0.0, 0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80]
    sweep: list[dict] = []
    ...
    for occ in occlusion_rates:
        ...
        for seed in SEEDS_3:
            test_data = _gen(TEST_SEQS, base_seed=seed + 55000, occlusion_rate=occ)
            pt_ip = sum(_eval_ip(pt, test_data)) / TEST_SEQS
            sa_ip = sum(_eval_ip(sa, test_data)) / TEST_SEQS
            pt_ips.append(pt_ip)
            sa_ips.append(sa_ip)
    ...
    if len(occ_arr) > 1:
        ...
        r_squared = 1 - (
            np.sum((pt_arr - (slope * occ_arr + intercept)) ** 2)
...
def bench_g6_1_static_stress() -> dict:
    ...
    for cfg in stress_configs:
        ...
        _p(
            f"    full: {np.mean(full_ips):.4f}, static: {np.mean(static_ips):.4f}, "
...
def bench_g7_1_trained_coherence() -> dict:
    ...
    for seed in SEEDS_7:
        ...
        with torch.no_grad():
            for seq in test_data:
                ...
                if len(phase_history) >= 2:
                    ...
                    # Binding persistence
                    bp = binding_persistence(res["identity_matches"], seq.n_objects)
                    all_persistence.append(bp)

        # Aggregate this seed
        mean_slip = (
            float(np.mean([s["slip_fraction"] for s in all_slips]))
            if all_slips
            else 0.0
        )
        ...
        mean_persistence = (
            float(np.mean([p["mean_persistence"] for p in all_persistence]))
            if all_persistence
            else 0.0
        )
    ...
    for key in [
        ...
        vals = [
            s[key]
            for s in per_seed
            if not (key == "coherence_half_life" and s[key] == float("inf"))
        ]
        if vals:
            agg[f"{key}_mean"] = float(np.mean(vals))
            agg[f"{key}_std"] = float(np.std(vals))
...
def bench_g8_1_pac_significance() -> dict:
    ...
    from prin.y4q1_tools import cross_frequency_coupling
    ...
    for seed in SEEDS_7:
        ...
        _p(
            f"    PAC={observed_mean:.4f}, null={null_mean:.4f}+/-{null_std:.4f}, "
...
def bench_preregistration() -> dict:
    ...
    protocol = {
        ...
        "gaps_addressed": [
            "G1_seed_expansion",
            "G2_pt_large_diagnostic",
            "G3_sa_breaking_point",
            "G4_chimera_tracking_bridge",
            "G5_fine_occlusion",
            "G6_static_stress",
            "G7_trained_coherence",
            "G8_pac_significance",
        ],
        "pac_surrogates": 1000,
        "alpha": 0.05,
    }
    h = hashlib.sha256(json.dumps(protocol, sort_keys=True).encode()).hexdigest()
    protocol["sha256"] = h
    _p(f"  SHA-256: {h[:16]}...")
...
def main() -> None:
    ...
    _p(f"\n{'=' * 60}")
    _p(
        f"Q1.9 complete: {completed} passed, {failed} failed "
