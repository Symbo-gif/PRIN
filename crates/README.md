...
| `prin-train` | Burn-based trainable layers, inhibition/STE, activations, HEP, optimizers. WP-022 delivered the first implementation: `bands::DiscreteDeltaThetaGamma` and `layers::ResonanceLayer` Burn `Module`s with validated `Config`/`Params`/`State` contracts, `TrainError`, gradient/parity/serialization tests, and a `strict-checks` numerical-guard feature | 4 |
