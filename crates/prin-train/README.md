...
### WP-031: Temporal experiments, statistics, and adversarial tooling (sessions 0121–0124)

- **`trainer` extensions** — `train_temporal_slot_attention_mot`/
  ...
  identical `TemporalTrainerConfig` (same Adam, warmup/cosine LR, gradient
  clipping, early stopping) for a fair PT-vs-SA comparison.
  `count_parameters` (generic over any `Module` via `ModuleVisitor`),
  `TrainingSnapshot`, and `train_multi_seed` (multi-seed statistical
  reliability aggregation). `train_phase_tracker` now threads `&mut Seed`
  explicitly (Project Plan §4 rule 3). PyO3 binding extended with `Seed`
  parameter and `snapshot_epochs`.
- **`temporal_metrics`** — tracking-quality metrics beyond identity
  preservation (port of PRINet 3.0 `utils/temporal_metrics.py`):
  `identity_switches`, `track_fragmentation`, `mostly_tracked_mostly_lost`,
  `track_duration_stats`, `recovery_speed`, `binding_robustness`,
  `temporal_smoothness`, and the `TemporalMetrics` aggregate. Every function
  cites exact reference line ranges in its rustdoc.
- **`stats`** — statistical utilities for rigorous multi-seed benchmarking:
  ...
  Student's-t p-value via log-gamma), `cohens_d` (effect size). Validated
  against real `scipy.stats.ttest_ind` 1.18.0: 8/8 scenarios at
  `rtol=1e-9, atol=1e-12` (`tests/parity_stats.rs`, fixture generated by
  `tools/wp031_stats_fixture.py`).
- **`flops`** — `LayerSpec`/`count_flops`/`FlopsReport` for FLOPs estimation
  per layer type (Linear, Conv2d, GRU, LayerNorm, BatchNorm2d, Embedding,
  MultiHeadAttention), plus `measure_wall_time`/`WallTimeStats` for
  empirical timing. One documented reference discrepancy (Conv2d docstring
  vs. implementation) resolved by reproducing the reference's *executed*
  behavior.
- **`adversarial`** — `fgsm_attack`/`pgd_attack` (L-infinity-bounded
  adversarial perturbation), per-tracker `adversarial_evaluate_pt`/
  ...
  Deterministic multi-seed behavior tested (`pgd_attack_is_deterministic_
  for_same_seed`/`..._different_seeds_give_different_perturbations`).
- **`error`** — extended with `InvalidBootstrapSamples`, `InvalidBootstrapCi`,
  `EmptySample`, `InvalidAttackEpsilon` variants.

S2 audit: PASS, zero findings (`DOCS/audits/031-wp031-audit.md`). S3:
no-change closure, delta re-audit CLEAN. Coverage ≥95% on every new file
